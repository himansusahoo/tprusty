...
class Rbac_action extends CI_Model {
    ...
    public function get_rbac_action_datatable($data = null, $export = null, $tableHeading = null, $columns = null) {
        ...
        if (!$columns) {
            ...
            $columns = 'action_id,name,code,module_name,status';
        }

        
        $this->datatables->select('SQL_CALC_FOUND_ROWS ' . $columns, false, false)
                ->from('rbac_actions_list_vw t1');
    ...
    public function get_rbac_action($columns = null, $conditions = null, $limit = null, $offset = null) {
        if (!$columns) {
            $columns = 'action_id,name,status,created,modified,code,module_name,module_id';
        }

        /*
         */
        $this->db->select($columns)->from('rbac_actions_list_vw t1');
     ...
     * @created:09/29/2018
     */
    public function get_module_options($columns, $index = null, $conditions = null)
    {
        if (!$columns) {
            $columns = 'module_id';
        }
     ...
     * isActionCodeExist
     * 
     * Validate action code is exist or not
     * 
     * @name 
     * @param String $code
     * @return integer
     ...
     */
    public function isActionCodeExist($code) {
        $query="SELECT count(code) total_count from rbac_actions where upper(code)=upper(?)";
        $result=$this->db->query($query,array($code))->row();
