...
/*----------------------------------- 12-oct-2017 ----------------------------------- */
	function select_oldnewsort_dataajax($search_title,$sort_val,$start_from)
	{
		$qq='$qq';
		$curl_strng=
		SOLR_BASE_URL.SOLR_CORE_NAME."/select?indent=on&q=".$search_title."&wt=json&sort=".$sort_val."&rows=50&start=".$start_from."";
			//echo $curl_strng;exit;		
			$curl2 = curl_init($curl_strng);
			curl_setopt($curl2, CURLOPT_RETURNTRANSFER, true);
			$output = curl_exec($curl2);
	...
	function select_oldnewsortajaxseg3($search_title,$sort_val,$start_from,$fqq)
	{
		...
		$curl_strng=
		SOLR_BASE_URL.SOLR_CORE_NAME."/select?indent=on&q=".$search_title."&wt=json&fq=".$fqq."&sort=".$sort_val."&rows=50&start=".$start_from."";
			}
			//echo $curl_strng;exit;		
			$curl2 = curl_init($curl_strng);
			curl_setopt($curl2, CURLOPT_RETURNTRANSFER, true);
