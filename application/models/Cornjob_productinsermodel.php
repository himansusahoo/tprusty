...
class Cornjob_productinsermodel extends CI_Model{
	...
	function select_newproductdata()
	{
		$this->cronjobsearch_skuprodid_insert();
		$this->category_update();
		$this->cronjobsearch_prodnameimage_update();
	...
	function select_productdataexistingproductapprove()
	{
		$this->cronjobsearch_skuprodid_insert();
		$this->category_update();
		$this->cronjobsearch_prodnameimage_update();
	...
	function existing_productdataupdateincronjobsearch($sku)
	{
				$cron_brand='';
				$cron_color='';
				$cron_size='';
				...
				$qr_brand=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Brand' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_brand->num_rows()>0)
				{@$cron_brand=$qr_brand->row()->attr_value;}	
				
				$qr_color=$this->db->query("SELECT *  FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Color' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_color->num_rows()>0)
				{@$cron_color=$qr_color->row()->attr_value;}
				
				$qr_size=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Size' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_size->num_rows()>0)
				{@$cron_size=$qr_size->row()->attr_value;}	
				
				
				$qr_subsize=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Sub size' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_subsize->num_rows()>0)
				{@$cronsub_size=$qr_subsize->row()->attr_value;}	
				
				$qr_occ=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Occasion' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_occ->num_rows()>0)
				{@$cron_occasion=$qr_occ->row()->attr_value;}
				
				$qr_type=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Type' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_type->num_rows()>0)
				{@$cron_type=$qr_type->row()->attr_value;}
				
				$qr_capacity=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='Capacity' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_capacity->num_rows()>0)
				{@$cron_Capacity=$qr_capacity->row()->attr_value;}
				
				$qr_RAM=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='RAM' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_RAM->num_rows()>0)
				{@$cron_RAM=$qr_RAM->row()->attr_value;}
				
				$qr_ROM=$this->db->query("SELECT * FROM seller_product_attribute_value a 
											INNER JOIN attribute_real b ON a.attr_id = b.attribute_id 
											WHERE b.attribute_field_name='ROM' AND a.sku='$sku' GROUP BY sku ");	
				if($qr_ROM->num_rows()>0)
				{$cron_ROM=$qr_ROM->row()->attr_value;}
				
				$this->db->query("
				UPDATE cornjob_productsearch SET  
				brand='$cron_brand',
				color='$cron_color',
				size='$cron_size',
...
GROUP BY sku ) WHERE sku IN ($sku_strng)");
	...
	function manual_update_filtering_attribute_data($skr_arr){
		set_time_limit(0);
		$arr=array();
		foreach($skr_arr as $k=>$v){
			$arr[] = "'".$v."'";
...
//						);
		...
		function update_globalcommisiontbl()
		{
			$qr=$this->db->query("SELECT * FROM category_indexing WHERE category_id NOT IN (SELECT cat_id FROM global_commission) AND `parent_id` IN (SELECT category_id FROM category_indexing WHERE `parent_id` IN (SELECT category_id FROM category_indexing WHERE `parent_id`=0) )");
			
			foreach($qr->result_array() as $res)
			{
				$data=array('cat_id'=>$res['category_id']);	
