...
//		$dt=date('Y-m-d H:i:s');
		...
		foreach($order_id_arr as $ky_ord=>$val_ord)
		{
			...
			{$this->db->query("UPDATE order_info SET Total_amount='$total_price' WHERE order_id='$val_ord' ");}*/
			
			$qrprod_pricesum=$this->db->query("SELECT sub_total_amount FROM ordered_product_from_addtocart WHERE order_id='$val_ord' ");
			$prod_totpricesum=0;
			foreach($qrprod_pricesum->result_array() as $res_prod_pricesum)
			{$prod_totpricesum=$prod_totpricesum+$res_prod_pricesum['sub_total_amount'];}
			
			$this->db->query("UPDATE order_info SET Total_amount='$prod_totpricesum' WHERE order_id='$val_ord' ");
		}
			...
			//---------------------------COD Data from session end-------------------//
				...
				/* for($ordctr=0; $ordctr<$order_idctr; $ordctr++)
				 { 		
				  ...
				  $orderid_unique=array_unique($order_id_arr);
				  $order_idctr=count($orderid_unique);
				 $cod_totalprice_arr=array();
				  for($ordctr=0; $ordctr<$order_idctr; $ordctr++)
				  {  
					  $cod_orderno=$orderid_unique[$ordctr];
					  
					  $qr_totprice=$this->db->query("SELECT Total_amount FROM order_info WHERE order_id='$cod_orderno' ");
					
					 $total_price=$qr_totprice->row()->Total_amount;
										  	
				      $qr_orderedproduct=$this->db->query("SELECT * FROM ordered_product_from_addtocart WHERE order_id='$cod_orderno' ");			  
					  
					  $prod_weight=array();
					  
					 foreach($qr_orderedproduct->result_array() as $res_singleorderid)
					 {
						$qnt_prod=$res_singleorderid['quantity'];
						$sku_orderedproduct=$res_singleorderid['sku'];
						
						$qr_product_master=$this->db->query("SELECT b.weight FROM product_master a INNER JOIN product_general_info b 
																 ON a.product_id=b.product_id WHERE a.sku='$sku_orderedproduct' GROUP BY a.sku  ");					
						
						
						for($i=1; $i<=$qnt_prod; $i++)
						{ 
										 
							$prod_weight[]=$qr_product_master->row()->weight;														 		
						}
						
										 
					 }// for loop end as each order with repeated in ordered_product_from_addtocart table
						...
						$total_weightcharge=0; 
						$total_taxchrgetobuyer=0; 
						$total_chargetocustomer=0; 
						$total_chargetomoonboy=0; 
						$totatl_discounttobuyer=0;
						...
						for($codj=0;$codj<count($prod_weight); $codj++)
						{
									...
									if($wtchrg_query->num_rows()>0)
									{
										...
										//-------------------amount to be charge start--------------------------
												...
												{$cod_totalprice=$cod_totalprice+$cod_chargeaswtgheach;$cod_totalprice_arr[]=$cod_totalprice;}
							...
							} // for loop end for weight charge to buyer
							...
							//--------------------------tax charge to buyer start--------------------
										...
										if($codtax_query->num_rows()>0)
										 ...
										 {$cod_totalprice=$cod_totalprice+$cod_chargetobuyer;$cod_totalprice_arr[]=$cod_totalprice;} 
					...
					//--------------------------tax charge to buyer end----------------------
						...
						if($cod_chargetobuyer!=0 && $codtaxamount!=0)
						{
							...
							$cod_totalprice_arr[]=$cod_totalprice;
					...
					} // if conditon end for COD discount case
					
					
					
					$data_codinfo=array(
								'order_id'=>$cod_orderno,
								'wt_charge'=>$total_weightcharge,
								'tax'=>$total_taxchrgetobuyer,
								'charge_tobuyer'=>$total_chargetocustomer,
								'charge_tomoonboy'=>$total_chargetomoonboy,
								'discount_amount'=>$totatl_discounttobuyer
															
							);
						 
						 	$this->db->insert('cod_transaction_log',$data_codinfo);
							$cod_total_price=$cod_totalprice_arr[$ordctr];
							$this->db->query("UPDATE order_info SET Total_amount='$cod_total_price' WHERE order_id='$cod_orderno' ");
											
				 
				  }  // for loop end as no of orders for product weight
				  
				  
				//print_r($cod_totalprice_arr);exit;
	...
	function insert_myorderdata($addtocart_ids,$order_id_arr,$tax_arr,$shipping_fees_arr,$sub_total_arr,$qantity_arr,$sku_arr,$total_price,$seller_id_arr,$address_id,$price_arr,$color_arr,$size_arr)
	{
		...
		{	
			...
			if($ct_row==0)
			{
					...
					$qrprod_pricesum=$this->db->query("SELECT sub_total_amount FROM ordered_product_from_addtocart WHERE order_id='$cod_orderid' ");
					$prod_totpricesum=0;
					foreach($qrprod_pricesum->result_array() as $res_prod_pricesum)
					{$prod_totpricesum=$prod_totpricesum+$res_prod_pricesum['sub_total_amount'];}
					
					$this->db->query("UPDATE order_info SET Total_amount='$prod_totpricesum' WHERE order_id='$cod_orderid' ");			
