			...
			//------------------------------Data delete form cart start---------------------------------------
				...
				//------------------------------COD transaction as order wise start----------------------------
				 ...
				 { 		
				 		...
				 		//$totprice_orderwise=$this->db->query("SELECT sum(sub_total_amount) as sub_total_amount FROM ordered_product_from_addtocart WHERE order_id='$cod_orderno' ");
						
					$totprice_orderwise=$this->db->query("SELECT * FROM ordered_product_from_addtocart WHERE order_id='$cod_orderno' "); //dt:21-01-2017
					$cod_totalprice=$totprice_orderwise->row()->sub_total_amount;
						
						
						
						
						$order_codquery=$this->db->query("SELECT product_id,quantity FROM ordered_product_from_addtocart WHERE order_id='$cod_orderno' ");
						$rw_order_cod=$order_codquery->result_array();
						
						foreach($rw_order_cod as $res_ordercod)
						{   $prod_weight=array();
							$cod_productid=$res_ordercod['product_id'];
							$cod_wtghquery=$this->db->query("SELECT weight FROM product_general_info WHERE product_id='$cod_productid' GROUP BY  product_id");
							$rw_wtgh=$cod_wtghquery->row();
							
							$constprodweight=$rw_wtgh->weight;
							
						
								$rec_ct=$res_ordercod['quantity'];
								if($rec_ct>1)
							   	{ 
								...
								{array_push($prod_weight,$rw_wtgh->weight);}							

						 } // inner for loop end of multiple sku of single order
						 
						 
						 //---------------------------------------COD Charge amount distribution start------------------------						 
									$cod_chargeaswtgh=0;
									$cod_chargetobuyer=0;
									$codtaxamount=0;
									$cod_chargeaswtgheach=0;
									$cod_chargeaswtghtotal=0;
									$cod_taxtotal=0;
									$cod_totalchargetobuyer=0;
									$cod_totaldiscountamount=0;
									$cod_totalamountchargetomoonboy=0;
						...
						for($codj=0;$codj<count($prod_weight); $codj++)
						{
							...
							if($wtchrg_query->num_rows()>0)
							{
								...
								$cod_chargeaswtghtotal=$cod_chargeaswtghtotal+$cod_chargeaswtgheach;
								//-------------------amount to be charge start--------------------------
										...
										if($cod_percen_query->num_rows()>0)
										{
											...
											$cod_percentagechargetomoonboy=100-$cod_percen_charge;
											$cod_amountchargetomoonboy=round(($cod_chargeaswtgheach/100)*$cod_percentagechargetomoonboy);
											
											$cod_totalamountchargetomoonboy=$cod_totalamountchargetomoonboy+$cod_amountchargetomoonboy;
											//$cod_chargetobuyer=$cod_chargetobuyer+round($codtaxamount);		
											$cod_totalchargetobuyer=$cod_totalchargetobuyer+$cod_chargetobuyer;
													...
													//if($cus_data!=""){
														...
														if($codtax_query->num_rows()>0)
														 { 
															...
															$codtaxamount=$codtaxamount+($cod_totalprice/100)*$cod_taxpercentage;															
															
															
															$cod_chargetobuyer=$cod_chargetobuyer+$codtaxamount;															
															
																													
															$cod_taxtotal=$cod_taxtotal+$codtaxamount;
							...
							if($cod_chargetobuyer!=0 && $codtaxamount!=0)
							{
								...
								if($cod_discountquery->num_rows()>0)
								{
									...
									$cod_totaldiscountamount=$cod_totaldiscountamount+$cod_discountamount;
							...
							$this->db->query("UPDATE order_info SET Total_amount='$cod_totalprice' WHERE order_id='$cod_orderno' ");
