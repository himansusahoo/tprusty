...
//}
    ...
    function search_product_clause_formobile($keyword) {
        ...
        if (strpos($fnl_sugst_word, ' ') > 0) {
            $condition1 = '';
            $condition2 = '';
            $spc_strngarr = explode(' ', $fnl_sugst_word);
            $ky_ctr = count($spc_strngarr);
            $i_ctr = 1;
            foreach ($spc_strngarr as $ky => $val) {
                if ($i_ctr == $ky_ctr) {
                    $or = '  ';
                } else {
                    $or = ' AND ';
                ...
                $condition1 = $condition1 . "(lvl2_name LIKE '%" . $val . "%' OR   lvl1_name LIKE '%" . $val . "%' OR  lvlmain_name LIKE '%" . $val . "%' )" . $or;
                $condition2 = $condition2 . "(lvl2_name LIKE '%" . $val . "%' OR   lvl1_name LIKE '%" . $val . "%' OR  lvlmain_name LIKE '%" . $val . "%' OR  name LIKE '%" . $val . "%' )" . $or;
                $i_ctr++;
            }

            $qr2 = $this->db->query("select DISTINCT lvl2, lvl2_name, lvl1, lvl1_name from temp_category  WHERE (" . $condition1 . ") group by lvl2 LIMIT 5");

            if ($qr2->num_rows() == 0) {
                $qr2 = $this->db->query("select DISTINCT lvl2, lvl2_name, lvl1, lvl1_name, sku, product_id, name  from cornjob_productsearch b WHERE b.quantity>0 AND b.seller_id!=0 AND b.prod_status = 'Active' AND b.status = 'Enabled' AND b.seller_status = 'Active' AND ( " . $condition2 . " ) group by b.lvl2 DESC LIMIT 5");
            }
...
//				if($count_rows==0){				
    ...
    function insert_from_tempwishlist() {
        ...
        if ($product_id_arr != 0) {
            ...
            $query = $this->db->query("SELECT * FROM wishlist WHERE user_id='$user_id_wishlist' AND sku='$sku_arr'");
            $row = $query->num_rows();
            if ($row > 0) {
                return false;
            } else {
...
FROM seller_account_information
    ...
    function insert_inn_wishlist_temp($product_id, $sku) {
        ...
        $this->session->set_userdata('addtowishlisttemp_prdid', $product_id);
        $this->session->set_userdata('addtowishlist_tempsku', $sku);
...
//===========================Order status change log insert end======================================================//
    ...
    function retrive_indivisual_order_id_details($order_id) {
        ...
        if ($query->num_rows == 0) {
            $query = $this->db->query("SELECT a.order_id,a.order_processstatus,a.Total_amount,a.date_of_order,a.order_status,a.invoice_id, a.order_id_payment_gateway,d.*,f.seller_id,f.business_name FROM order_info a 
			INNER JOIN ordered_product_from_addtocart e ON a.order_id=e.order_id
			INNER JOIN seller_account_information f ON e.seller_id=f.seller_id 
			INNER JOIN shipping_address d ON a.order_id=d.order_id WHERE a.order_id='$order_id'");
