...
WHERE a.product_id
IN ($prodid_str) AND h.indexing_status='Pending' AND h.prod_process_status='Add'
GROUP BY b.sku  ORDER BY a.product_id DESC  ");
	...
	function addsolr_indesing()	
	{
		...
		foreach($query->result_array() as $res_prod)
		{
			...
			/*$sts_disp="<fieldset style='background-color:rgb(193, 226, 217); border: 1px solid; border-radius: 5px; text-align:center; height:75px;'><legend style='background-color:#CCC;'>Indexed Status</legend>
SKU:<span style='font-weight:bold;'> ".$res_prod['sku']." </span> &nbsp;
<img src='". base_url()."images/success_icon.png'> Indexed in Solr Server Successfully. 
			...
			/*else
			{
				$sts_disp="<fieldset style='background-color:rgb(193, 226, 217); border: 1px solid; border-radius: 5px; text-align:center; height:75px;'><legend style='background-color:#CCC;'>Indexed Status</legend>
SKU:<span style='font-weight:bold;'> ".$res_prod['sku']." </span> &nbsp;
<img src='". base_url()."images/error.png'> Indexed in Solr Server Failed. 
	...
	{
		$qr=$this->db->query("SELECT count(sql_id) as tot FROM solar_indexing WHERE (indexing_status='Pending' OR indexing_status='Completed') AND prod_process_status='Edit' ");
		//return $qr->num_rows();
...
INNER JOIN solar_indexing h on h.sku=b.sku
WHERE a.product_id
IN ($prodid_str) AND (h.indexing_status='Pending' OR h.indexing_status='Completed') AND h.prod_process_status='Edit'
GROUP BY b.sku  ORDER BY a.product_id DESC  ");
	...
	function updatesolr_indesing()
	{
		...
		$query=$this->db->query("select distinct a.product_id,a.name,a.sku,a.lvl2_name,a.lvl1_name,lvlmain_name,a.brand,
                                 a.color,a.size,a.Capacity,a.RAM,a.ROM,a.seller_id FROM cornjob_productsearch a
								 INNER JOIN solar_indexing b ON a.sku=b.sku  
								 WHERE (b.indexing_status='Pending' OR b.indexing_status='Completed') AND prod_process_status='Edit' AND b.sku IN ($skuids_strg) group by a.sku    ");
		if(base_url()=='https://www.moonboy.in/')
		{$solr_colection='mycollection1';}
		else
		{
