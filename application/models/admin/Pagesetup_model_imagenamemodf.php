    ...
    function add_pagesectioninfo() {
        ...
        for ($i_clmn = 0; $i_clmn < $col_num; $i_clmn++) {
            ...
            for ($s = 0; $s < $fileCount; $s++) {
                ...
                $fil_nm = trim(str_replace(' ', '', preg_replace('/\s*\([^)]*\)/', '', preg_replace('#"#', '', preg_replace("/'/", '', preg_replace('#/#', '', basename($files["userfile" . $i_img . ""]['name'][$s])))))));

                $_FILES['userfile']['name'] = $fil_nm;
                ...
                if ($img_skuorurl[$s] == 'sku') {
                    ...
                    $fil_nm = trim(str_replace(' ', '', preg_replace('/\s*\([^)]*\)/', '', preg_replace('#"#', '', preg_replace("/'/", '', preg_replace('#/#', '', basename($files["userfile_excel" . $i_img . ""]['name'][$s])))))));
            ...
            for ($s = 0; $s < count($imgsqlid_hdn); $s++) {
                ...
                if ($img_sqlid != '') {
                    ...
                    if ($img_skuorurl[$s] == 'sku') {
                        ...
                        if ($fil_nm != '') {  //$this->load->model('PHPExcel/Phpexcel_iofactory');
                            $this->load->library('upload');
                            $dt_img = preg_replace("/[^0-9]+/", "", date('Y-m-d H:i:s'));
                            $excel_rndname = strtolower(random_string('alnum', 15)) . $dt_img . '.csv';
