...
class Desktop_searchpage_model extends CI_Model {
    ...
    function select_imgsize() {
        ...
        $qr = $this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='desktop' AND page_nm='searchpage' AND culumns_count='$clmn_conunt' AND " . $screeshot_clmnname . "!=''  ");
    ...
    function add_pagesectioninfo() {
        ...
        $qr_seciontbl = $this->db->query("SELECT max(Sec_id) as maxi_secid FROM desktopsite_pagesectioninfo WHERE page_id='5' AND page_name='searchpage' ");
        ...
        $qr_secionorderby = $this->db->query("SELECT max(Order_by) as maxi_orderby FROM desktopsite_pagesectioninfo WHERE page_id='5' AND page_name='searchpage' ");
        ...
        for ($i_clmn = 0; $i_clmn < $col_num; $i_clmn++) {
            ...
            $qr_clmnid = $this->db->query("SELECT max(clmn_id) as maxiclmn_id FROM desktopsite_columninfo 
											WHERE page_id='5' AND sec_id='$updated_secid'  ");
            ...
            for ($s = 0; $s < $fileCount; $s++) {
                ...
                $qr_clmnsqlid = $qr_clmnordby = $this->db->query("SELECT clmn_sqlid FROM desktopsite_columninfo 
											WHERE page_id='5' AND sec_id='$updated_secid' AND clmn_id='$updated_clmnid'  ");
    ...
    function update_pagesectioninfo() {
        ...
        $this->db->query("UPDATE desktopsite_pagesectioninfo SET 
								...
								sec_descrp='$sec_descrp'
								WHERE page_id='5' AND page_name='searchpage' AND Sec_id='$Sec_id'
        ...
        for ($i_clmn = 0; $i_clmn < $col_num; $i_clmn++) {
            ...
            for ($s = 0; $s < count($imgsqlid_hdn); $s++) {
                ...
                if ($img_sqlid != '') {
                    ...
                    $this->db->insert('desktopsite_imageinfo', $data_imginfo);
                } // if image row not in desktopite_imageinfo table
                //------ image data upload other data like sku, memo insert end------//
            }
            //--------  multiple image for loop upload end------------//			
            $i_img++;
    ...
    function section_dataof_desktopsearchpage() {
        $qr_section = $this->db->query("SELECT * FROM desktopsite_pagesectioninfo WHERE page_id='5' AND page_name='searchpage' ORDER BY Order_by ");
        return $qr_section;
    }

    function searchpagesectionsorby_up() {
        ...
        $qr_sectionperv = $this->db->query("SELECT pgsection_sqlid FROM desktopsite_pagesectioninfo WHERE page_id='5' AND page_name='searchpage' AND Order_by='$order_byprevsec' ");
    ...
    function searchpagesectionsorby_down() {
        $sec_sqlid = $this->uri->segment(4);
        ...
        $qr_sectionnext = $this->db->query("SELECT pgsection_sqlid FROM desktopsite_pagesectioninfo WHERE page_id='5' AND page_name='searchpage' AND Order_by='$order_bynextsec' ");
    ...
    function searchpagesortby_section_movetop() {
        ...
        $sql_sel_data = $this->db->query("SELECT MIN(Order_by)Order_by FROM desktopsite_pagesectioninfo WHERE page_id='5'");
    ...
    function searchpagesortby_section_movedown() {
        ...
        $sql_sel_data = $this->db->query("SELECT MAX(Order_by) Order_by FROM desktopsite_pagesectioninfo WHERE page_id='5'");
        $exit_data = $sql_sel_data->row()->Order_by;
    ...
    function searchpage_sectioneditedpage() {
        $sec_sqlid = $this->uri->segment(4);
    ...
    function delete_desktopsearchpage_section() {
        $output_dir = "./images/pagedesign_image/";
        $sec_sqlid = $this->uri->segment(4);
