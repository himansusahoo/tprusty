...
class Pagecatlog_model extends CI_Model {
	...
	public function select_imgsize()
	{
		...
		$qr=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='mobile' AND page_nm='catalog' AND culumns_count='$clmn_conunt' AND ".$screeshot_clmnname."!='' ");
	...
	{
		$menu_link_id=$this->input->post('menu_link');
		...
		$qr_seciontbl=$this->db->query("SELECT max(Sec_id) as maxi_secid FROM mobilesite_pagesectioninfo WHERE page_id='3' AND page_name='catalog' ");
		...
		$qr_secionorderby=$this->db->query("SELECT max(Order_by) as maxi_orderby FROM mobilesite_pagesectioninfo WHERE page_id='3' AND page_name='catalog' ");
		...
		{$updated_ordby=1;}
			...
			//-----------------------------start: update product catalog section if this section exist----------------------------------//
			
			$qr_prodcatlog=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo 
			                                WHERE page_id='3' AND page_name='catalog' AND sec_type='Product Catalog' ");
			$update_menulink=array();								
			if($qr_prodcatlog->num_rows()>0)
			{	
				$prod_menulink=$qr_prodcatlog->row()->menu_id;
				$prod_menulinkarr=unserialize($prod_menulink);
				
				$update_menulink=array_merge($prod_menulinkarr,$menu_link_id);
				
				$newupdate_menulinksrlz=serialize(array_unique($update_menulink)); 
				
				$this->db->query("UPDATE mobilesite_pagesectioninfo SET menu_id='$newupdate_menulinksrlz'  
				                  WHERE page_id='3' AND page_name='catalog' AND sec_type='Product Catalog'");
					
			}								
			//-----------------------------end: update product catalog section if this section exist------------------------------------//
			...
			for($i_clmn=0; $i_clmn<$col_num;  $i_clmn++)
			{
				...
				$qr_clmnid=$this->db->query("SELECT max(clmn_id) as maxiclmn_id FROM mobilesite_columninfo 
											WHERE page_id='3' AND sec_id='$updated_secid'  ");
					...
					for($s=0; $s<$fileCount; $s++){
						...
						if($ext=="gif"){
							...
							$qr_clmnsqlid=$qr_clmnordby=$this->db->query("SELECT clmn_sqlid FROM mobilesite_columninfo 
											WHERE page_id='3' AND sec_id='$updated_secid' AND clmn_id='$updated_clmnid'  ");
	...
	{	
		...
		$exit_menudata=$this->db->query("SELECT menu_id FROM mobilesite_pagesectioninfo WHERE pgsection_sqlid='$pg_id' ");
		$check_menulinkid='';
		...
		//-----------------------------start: update product catalog section if this section exist----------------------------------//
			if($menu_link!='')
			{ $menu_link_ids=unserialize($menu_link);
				$qr_prodcatlog=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo 
												WHERE page_id='3' AND page_name='catalog' AND sec_type='Product Catalog' ");
				$update_menulink=array();								
				if($qr_prodcatlog->num_rows()>0)
				{	
					...
					$update_menulink=array_merge($prod_menulinkarr,$menu_link_ids);
		...
		$sec_brcolor_checked=$this->input->post('secclr_checked');
			...
			$this->db->query("UPDATE mobilesite_pagesectioninfo SET bg_color='$final_secbgcolor', sec_type='$section_type',menu_id='$menu_link',
								sec_type_data='$sectiondata_type', lst_updt_dt_tm='$dtm', memo='$sec_memo', Status='$section_status',
								image_size='$slctimage_size',sec_lbl='$section_label',sec_descrp='$sec_descrp' WHERE page_id='3' AND page_name='catalog' AND Sec_id='$Sec_id' ");
	...
	public function catlog_sectioneditedpage()
	{
	...
	public function section_dataof_mobilecatlog()
	{
		$qr_section=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo WHERE page_id='3' AND page_name='catalog' ORDER BY Order_by ");
		return $qr_section;
	}
	
	function select_pagedesinginfo_as_menuwise()
	{	$pgsectionsqlid_strng='';
		$menu_linkid=$this->input->post('mnulinkid');
		$qr_section=$this->db->query("SELECT pgsection_sqlid,menu_id FROM mobilesite_pagesectioninfo WHERE page_id='3' AND page_name='catalog' AND menu_id!='' ORDER BY Order_by ");
		...
		$qr_section=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo 
		                              WHERE page_id='3' AND page_name='catalog' AND pgsection_sqlid IN ($pgsectionsqlid_strng)
									   ORDER BY Order_by ");
		}		
		else
		{$qr_section=false;}
	...
	public function catlogsectionsorby_up()
	{
		//$sec_sqlid=$this->uri->segment(4);
		...
		$qr_sectionperv=$this->db->query("SELECT pgsection_sqlid FROM mobilesite_pagesectioninfo WHERE page_id='3' AND page_name='catalog' AND Order_by='$order_byprevsec' ");		
	...
	public function catlogsectionsorby_down()
	{
		//$sec_sqlid=$this->uri->segment(4);
		$sec_sqlid=$this->input->post('pgsec_sqlid');
		...
		$qr_sectionnext=$this->db->query("SELECT pgsection_sqlid FROM mobilesite_pagesectioninfo WHERE page_id='3' AND page_name='catalog' AND Order_by='$order_bynextsec' ");		
	...
	function  catlogsectionsorby_movetop()
	{
		$sec_sqlid=$this->input->post('pgsec_sqlid');
		
		$sql_sel_data=$this->db->query("SELECT MIN(Order_by)Order_by FROM mobilesite_pagesectioninfo WHERE page_id='3'");
	...
	function  catlogsectionsorby_movedown()
	{
		$sec_sqlid=$this->input->post('pgsec_sqlid');
		
		$sql_sel_data=$this->db->query("SELECT MAX(Order_by) Order_by FROM mobilesite_pagesectioninfo WHERE page_id='3'");
		$exit_data=$sql_sel_data->row()->Order_by;
	...
	public function delete_mobilecatlog_section()
	{ $output_dir = "./images/pagedesign_image/";
		//$sec_sqlid=$this->uri->segment(4);
		$sec_sqlid=$this->input->post('sec_sqlid');
	...
	{	
		$qr=$this->db->query(" SELECT * FROM category_menu_mobile WHERE  parent_id IN
							(SELECT dskmenu_lbl_id FROM category_menu_mobile WHERE 
							parent_id IN (SELECT dskmenu_lbl_id FROM category_menu_mobile WHERE
							 parent_id=0 ) )  ORDER BY dskmenu_lbl_id");
	...
	public function remove_menulink()
		{
			...
			$qr_extcatg=$this->db->query("SELECT menu_id FROM mobilesite_pagesectioninfo WHERE pgsection_sqlid='$sec_id' ");			
			...
			$this->db->query("UPDATE mobilesite_pagesectioninfo SET menu_id='$pccatg_updatedList' WHERE pgsection_sqlid='$sec_id' ");			
		}
}
?>