...
class Pagecategory_model extends CI_Model {
	...
	public function select_imgsize()
	{
		...
		$qr=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='mobile' AND page_nm='category' AND culumns_count='$clmn_conunt' AND ".$screeshot_clmnname."!='' ");
		return 	$qr;
	}
	...
	public function add_pagesectioninfo()
	{
		$menu_link_id=$this->input->post('menu_link');
		  /*foreach($menu_link_id as $key=>$val)
		  {
			  $cacheqr=$this->db->query("SELECT url_displayname FROM `category_menu_mobile` WHERE `dskmenu_lbl_id`='$val' ");
			  $folder_name=$cacheqr->row()->url_displayname;
			  $this->db->cache_delete('category', $folder_name);
		  }*/
		//print_r($menu_link_id);
		...
		$col_num=$this->input->post('col_num');
		$set_desc=$this->input->post('richtxteditor_data');
		...
		$qr_seciontbl=$this->db->query("SELECT max(Sec_id) as maxi_secid FROM mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' ");
		...
		$qr_secionorderby=$this->db->query("SELECT max(Order_by) as maxi_orderby FROM mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' ");
		...
		{$updated_ordby=1;}
			...
			for($i_clmn=0; $i_clmn<$col_num;  $i_clmn++)
			{
				...
				$qr_clmnid=$this->db->query("SELECT max(clmn_id) as maxiclmn_id FROM mobilesite_columninfo 
											WHERE page_id='2' AND sec_id='$updated_secid'  ");
					...
					for($s=0; $s<$fileCount; $s++){
						...
						if($ext=="gif"){
							...
							$qr_clmnsqlid=$qr_clmnordby=$this->db->query("SELECT clmn_sqlid FROM mobilesite_columninfo 
											WHERE page_id='2' AND sec_id='$updated_secid' AND clmn_id='$updated_clmnid'  ");
	...
	{		
		...
		$sec_brcolor_checked=$this->input->post('secclr_checked');
			...
			$this->db->query("UPDATE mobilesite_pagesectioninfo SET bg_color='$final_secbgcolor', sec_type='$section_type', menu_id='$menu_link',
								sec_type_data='$sectiondata_type', lst_updt_dt_tm='$dtm', memo='$sec_memo', Status='$section_status', sec_descrp='$set_desc',
								image_size='$slctimage_size',sec_lbl='$section_label' WHERE page_id='2' AND page_name='category' AND Sec_id='$Sec_id' ");
	...
	public function category_sectioneditedpage()
	{
	...
	public function section_dataof_mobilecategory()
	{
		$qr_section=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' ORDER BY Order_by ");
		return $qr_section;
	}
	
	function select_pagedesinginfo_asmenuwise()
	{	$pgsectionsqlid_strng='';
		$menu_linkid=$this->input->post('mnulinkid');
		$qr_section=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' AND menu_id!='' ORDER BY Order_by  ");
		...
		$qr_section=$this->db->query("SELECT * FROM mobilesite_pagesectioninfo 
		                             WHERE pgsection_sqlid IN ($pgsectionsqlid_strng) ORDER BY Order_by ");
	...
	public function categorysectionsorby_up()
	{
		//$sec_sqlid=$this->uri->segment(4);
		...
		$qr_sectionperv=$this->db->query("SELECT pgsection_sqlid FROM mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' AND Order_by='$order_byprevsec' ");		
	...
	public function categorysectionsorby_down()
	{
		//$sec_sqlid=$this->uri->segment(4);
		$sec_sqlid=$this->input->post('pgsec_sqlid');
		$qr_section=$this->db->query("SELECT Order_by FROM mobilesite_pagesectioninfo WHERE pgsection_sqlid='$sec_sqlid' ");		
		...
		$qr_sectionnext=$this->db->query("SELECT pgsection_sqlid FROM mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' AND Order_by='$order_bynextsec' ");		
	...
	function  categorysectionsorby_totop()
	{
		$sec_sqlid=$this->input->post('pgsec_sqlid');
		
		$sql_sel_data=$this->db->query("SELECT MIN(Order_by) Order_by FROM mobilesite_pagesectioninfo WHERE page_id='2'");
	...
	function  categorysortby_section_todown()
	{
		$sec_sqlid=$this->input->post('pgsec_sqlid');
		
		$sql_sel_data=$this->db->query("SELECT MAX(Order_by) Order_by FROM mobilesite_pagesectioninfo WHERE page_id='2'");
		$exit_data=$sql_sel_data->row()->Order_by;
	...
	public function delete_mobilecategory_section()
	{ $output_dir = "./images/pagedesign_image/";
		//$sec_sqlid=$this->uri->segment(4);
	...
	{	
		$qr=$this->db->query("SELECT * FROM category_menu_mobile WHERE parent_id IN (SELECT dskmenu_lbl_id FROM category_menu_mobile WHERE parent_id=0 )  ORDER BY dskmenu_lbl_id");
		return $qr;
	}
