...
//		INNER JOIN seller_account i ON h.seller_id=i.seller_id AND i.status='Active' where ".$condition." ORDER BY a.seller_product_id DESC LIMIT ".$start.", ".$limit."");
	...
	function retrive_seller_product_exiting_data(){
		...
		INNER JOIN seller_account g ON f.seller_id=g.seller_id WHERE a.status='Active' AND (g.status='Active' OR g.status='Pending')  AND f.approve_status='Pending' ORDER BY f.seller_exist_product_id DESC");
