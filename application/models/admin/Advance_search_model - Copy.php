...
class Advance_search_model extends CI_Model {
	...
	//--------------------product add or modified  date initialize start -----------------//	
				...
				if($prod_prcdiscntttype!='')
				{
					...
					{ $j_prodprice=0;
						foreach($pricefrom_arr as $ky_price=>$val_price)
						{
							...
							$pricefromto_string=$pricefromto_string.' (current_price>='."'".$val_price."'".' AND current_price<='."'".$priceto_arr[$ky_price]."'".') '.$or.' ';
							$j_prodprice++;	
						}
					...
					{ $j_prodprice=0;
						foreach($discountfrom_arr as $ky_discount=>$val_discount)
						{
							...
							$discountfromto_string=$discountfromto_string.' ((100-(current_price*(100/mrp)))>='."'".$val_discount."'".' AND (100-(current_price*(100/mrp)))<='."'".$discountto_arr[$ky_discount]."'".') '.$or.' ';
							$j_prodprice++;	
						}	
					} 
		...
		//-------------------------------------- query for attribute value start--------------------------------------------//
			...
			if($seller_idsrchstring!='' && $catg_ids_string!='' && $attrbgrouids_string!='' && $attrbvalueasqlis_string!='' && $addfromto_date_string=='' 					 && $modffromto_date_string=='')
			{
				 ...
				 $qr=$this->productdata_asattributrvalue($product_status,$seller_idsrchstring,$catg_ids_string,$attrbgrouids_string,$attrbvalueasqlis_string,$actualattrbid_string,$actualattrbvalue_string,$last_segment,$limit,$start);			
		...
		if($seller_idsrchstring!='' && $catg_ids_string!='' && $attrbgrouids_string!='' && $addfromto_date_string=='' && $modffromto_date_string=='')
		{
			...
			SELECT a.seller_product_id AS product_id, a.sku, a.name, b.product_approve AS prod_status, c.catelog_img_url AS imag,f.business_name,
					e.mrp,e.price,e.special_price asspecial_price, e.price_fr_dt as special_pric_from_dt , e.price_to_dt as special_pric_to_dt
										...
										AND b.attribut_set IN ($attrbgrouids_string) 
										AND b.seller_id IN ($seller_idsrchstring)
		...
		if($pricefromto_string!='' && $discountfromto_string!='')
		{
			...
			a.special_pric_from_dt,a.special_pric_to_dt,b.business_name FROM cornjob_productsearch a
			INNER  JOIN seller_account_information b ON a.seller_id=b.seller_id			
			WHERE ($pricefromto_string) and a.sku!=''  AND a.prod_status='Active' AND a.status='Enabled' AND a.seller_status='Active'  	GROUP BY a.sku  LIMIT ".$start.", ".$limit." 
			
			UNION ALL
			
			SELECT product_id,a.sku,a.name,prod_status,imag,a.mrp,a.price,a.special_price,
			a.special_pric_from_dt,a.special_pric_to_dt,b.business_name FROM cornjob_productsearch a
			INNER  JOIN seller_account_information b ON a.seller_id=b.seller_id			
			...
			a.special_pric_from_dt,a.special_pric_to_dt,b.business_name FROM cornjob_productsearch a
			INNER  JOIN seller_account_information b ON a.seller_id=b.seller_id			
			WHERE ($pricefromto_string) and a.sku!='' AND (a.prod_status='Inactive' OR a.status='Disabled') GROUP BY a.sku  LIMIT ".$start.", ".$limit." 
	...
	function productdata_asattributrvalue($product_status,$seller_idsrchstring,$catg_ids_string,$attrbgrouids_string,$attrbvalueasqlis_string,$actualattrbid_string,$actualattrbvalue_string,$last_segment,$limit,$start)
	{ 		
			$catg_id=$catg_ids_string;
		
		if($product_status=='Active')
			{
				$qr=$this->product_attributefilter_as_seller($catg_id,$last_segment,$seller_idsrchstring,$product_status,$limit,$start);
			...
			else
			{
				...
				{
					$qr=$this->inactiveproduct_attributefilter_as_seller($catg_id,$last_segment,$seller_idsrchstring,$product_status,$limit,$start);
				}
				else
				{
					$qr=false;	
	...
	function product_attributefilter_as_seller($catg_id,$last_segmt,$seller_idsrchstring,$product_status,$limit,$start)
	{
	...
	function inactiveproduct_attributefilter_as_seller($catg_id,$last_segmt,$seller_idsrchstring,$product_status,$limit,$start)
	{
