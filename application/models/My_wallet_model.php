...
class My_wallet_model extends CI_Model {
    ...
    function calculating_total_adjustment_n_without_wallet() {
        ...
        if ($query->num_rows() > 0) {
            ...
            echo json_encode(array("rest_payble_amt" => $rest_payble_amt, "rest_payble_amt_without_wlt" => $rest_payble_amt_without_wlt));
            exit;
        } else {
            return false;
        }
        ...
        if ($total_wallet_amt > $wallet_payble_amt) {
            ...
            if ($rest_payble_amt < $wallet_payble_amt) {
                ...
                if ($sql->num_rows() > 0) {
                    ...
                    echo json_encode(array("wlt_adj_amt" => $final_wlt_adj_amt, "total_adj_amt" => $total_adj_amt));
                    exit;
                } else {
                    $temp_adj_data = array(
                    ...
                    $adj_res = $adj_sql->result();
                    $total_adj_amt = $adj_res[0]->TOTAL_ADJ_AMT;
                    echo json_encode(array("wlt_adj_amt" => $wallet_payble_amt, "total_adj_amt" => $total_adj_amt));
                    exit;
                }
            }
        } else {
