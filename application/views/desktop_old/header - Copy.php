...
<!-- End Google Tag Manager (noscript) -->
     ...
     <!-- Top Bar -->
            ...
            <div class="topbar-content">
					...
					<div  class="top-right">  
						<ul>
                        <li> <a class="blog" href="https://socialmoonboy.wordpress.com/" target="_blank"> Our Blog </a> </li>
       ...
       <div class="topbar-content">
              ...
              <div class="logo"> <a href="<?php echo base_url(); ?>"><img src="<?php echo base_url();?>images/logo.png" alt="moonboy"></a> </div>
...
<!--1st level-->
<?php $q_arrow=$this->db->query("select * from category_menu_desktop where parent_id='$rw->dskmenu_lbl_id' AND order_by!=0 AND is_active='Yes' order by order_by "); 
	  	
	  	$ct_arrow=$q_arrow->num_rows();		
	  	if($ct_arrow>0){ ?> <?php } ?> <i class="fa fa-angle-right"></i> </h1>
      ...
      <?php $qr=$this->db->query("select * from category_menu_desktop where parent_id='$rw->dskmenu_lbl_id' AND order_by!=0 AND is_active='Yes' order by order_by "); 
	  	//$rw=$qr->result();
	  	$ct=$qr->num_rows();
        ...
        <?php 
			foreach($qr->result() as $rs){ ?> <!--level-2 -->
                  ...
                  <?php
				  if($rs->view_type=='category'){ ?>
				    <a href="<?php echo base_url().'product_description/product_data/'.preg_replace('#/#',"-",str_replace(' ','-',strtolower($rs->label_name))).'/'.$rs->dskmenu_lbl_id ?>">               <?php }else{ ?>   
            <a href="<?php echo base_url().'product_description/category_catalog/'.preg_replace('#/#',"-",str_replace(' ','-',strtolower($rs->label_name))).'/'.$rs->dskmenu_lbl_id ?>"> 
             <?php } ?>
                <h1 class="catgry-name"><?php echo $rs->label_name;?>  
     
<?php $q_arrow1=$this->db->query("select * from category_menu_desktop where parent_id='$rs->dskmenu_lbl_id' AND order_by!=0 AND is_active='Yes' order by order_by "); 
	  	...
	  	if($ct_arrow1>0){ ?>
           ...
           <?php } ?> </h1> </a>
			...
			<div class="sub-menuitems">
                       ...
                       <?php $qr1=$this->db->query("select * from category_menu_desktop where parent_id='$rs->dskmenu_lbl_id' AND order_by!=0 AND is_active='Yes' AND category_id!='' order by order_by "); 
	  	//$rw=$qr->result();
	  	$ct1=$qr1->num_rows();
        ...
        <?php 
			...
			<li> <a href="<?php echo base_url().'product_description/product_addtocart/'.preg_replace('#/#',"-",str_replace(' ','-',strtolower($rs1->label_name))).'/'.str_replace(',','-',$rs1->category_id) ?>">  <?php echo	$rs1->label_name;?>
    
<?php $q_arrow2=$this->db->query("select * from category_menu_desktop where parent_id='$rs1->dskmenu_lbl_id' AND order_by!=0 AND is_active='Yes' order by order_by "); 
 ...
 <?php } ?> 
	   ...
	   ?>
        <input type="text" placeholder="Search Your Product" id="search-text" name="search" autocomplete="off" required/> 
        ...
        <li class="wlist-top" > <a href="
                    <?php if(@$this->session->userdata['session_data']['user_id']!=""){ echo base_url().'user/wishlist'; ?> " title="your Wishlist!" > <i class="fa fa-heart-o"></i>
					<?php 
       ...
       <div class="topbar-content">
        ...
        <li  class="cart-top" onMouseOver="OverFunction_cart()" onMouseOut="OutFunction_cart()"> 
             <?php /*?> <a href="<?php if(@$this->session->userdata['session_data']['user_id']!=""){ echo base_url().'mycart/mycart_detail'; }else {echo base_url().'mycart/guest_cart_detail'; } ?>" title="Show my cart"> <?php */?> 
              ...
              <i class="fa fa-shopping-cart"></i> 
              <div  id="top-cart_data">  <?php if(@$this->session->userdata('addtocarttemp_session_id')!="" && @$this->session->userdata['session_data']['user_id']==""){
  ...
  <tr>
    <td>
    	<div id="pass_dv1">
            ...
            <input type="password" name="ncpass" id="ncpass" class="input-text" Placeholder="Re-enter password" onKeyDown="if (event.keyCode == 13) document.getElementById('in_up').click()">
          
        </div>
        <div id="pass_dv2">
    		<input type="password" name="epass" id="epass" class="input-text" Placeholder="Enter password"  onkeydown="if (event.keyCode == 13) document.getElementById('in_up').click()">
