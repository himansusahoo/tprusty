...
<div class="line">&nbsp;</div>
  ...
  <tr>
   ...
   $image_cart=explode(',',$rw1->imag);
    ...
    ?> <a href="<?php echo base_url().'product_description/product_detail/'.preg_replace('#/#',"-",str_replace(" ","-",strtolower($rw1->name))).'/'.$rw1->product_id.'/'.$rw1->sku  ?>" target="_blank"><img src="<?php echo base_url().'images/product_img/'.'catalog_'.$image_cart[0]; ?>"  width="30" alt="<?=$rw1->name;?>"></a></div>
			...
			if($wtchrg_query->num_rows()>0)
			{
				...
				//-------------------amount to be charge start--------------------------
						...
						if($cod_percen_query->num_rows()>0)
						{
							...
							$cod_chargetobuyer=round(($cod_chargeaswtgheach/100)*$cod_percen_charge);
