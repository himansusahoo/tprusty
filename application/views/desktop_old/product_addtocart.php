       ...
       <?php /*?> <div class="f_sidebar">
		...
		<!-- Type filtering section Start-->
           ...
           <div class="f_sidebar">           
               ...
               <section class="filter-form">
				...
				<?php 
				if($this->uri->segment(4)!='' || $this->uri->segment(4)!='NOT')
		...
		?>
               ...
               <li title="<?php echo trim($res_attrbval['attr_value'])?>">
                ...
                <input  type="checkbox" id="attrb_realvalue<?=$iattrb?>" name="attrb_realvalue[]" value="<?=trim($res_attrbval['attr_value'])?>" 
                onChange="filter_attribute('<?=$label_name;?>','<?=$catgstr_urlpass;?>','<?php if($this->uri->segment(4) != ''){ echo $strng;}else{ echo 'NOT';}?>','<?=trim($res_attrbval['attr_id']).'-'.trim($valattrbhead);?>','<?=trim($res_attrbval['attr_value'])?>','<?=$iattrb?>')" <?php //if(strpos($strng,trim($res_attrbval['attr_value'])))
                ...
                <i></i>
				 ...
				 ?>
                       ...
                       <input type="checkbox" id="attrb_realvalue<?=$iattrb?>" name="attrb_realvalue[]" value="<?=trim($res_attrbval['attr_value'])?>" onChange="filter_attribute('<?=$label_name;?>','<?=$catgstr_urlpass;?>','<?php if($this->uri->segment(4) != ''){ echo $strng;}else{ echo 'NOT';}?>','<?=trim($res_attrbval['attr_id']).'-'.trim($valattrbhead);?>','<?=trim($res_attrbval['attr_value'])?>','<?=$iattrb?>')" 
...
<a href="<?php echo base_url() ?>" style="color:#666;">Home </a> > 
<a href="<?php  echo base_url().'category/'.$parent_bredcumnamelink ?>" style="color:#666;"><?=$parent_bredcumname?> </a> >
<a href="<?php echo base_url().$label_name ?>"><?=$bredcum_name?> </a>
<!-- Breedcum end--> 
...
<div class="w_content">
		<div class="catlog">
			...
			<?php if(@$arr1[1]=='Low-To-High'||  @$arr1[1]=='High-To-Low')   { echo " ";}else{ echo "  "?>	<span class="rst_spn" onClick="resetFilterIndivisually('<?=$label_name;?>','<?=$catgstr_urlpass;?>','<?=$this->uri->segment(4);?>','<?php echo $arr1[0].'='.$arr1[1];?>')"> <?php  echo str_replace('%20',' ',$arr1[1]); ?>&nbsp;<i class="fa fa-times close_filter" aria-hidden="true" ></i></span>&nbsp;&nbsp;&nbsp;&nbsp;
<?php }  ?>
                ...
                <!--<span>HP <i class="fa fa-times" aria-hidden="true"></i></span>-->
                 <div class="dropdown_left">
			       Sort By Price &nbsp;<select class="dropdown selectpicker" id="attr_size"  data-style="btn-info" style="width:auto;" onChange="sortby_price(this.value,'<?=$label_name;?>','<?=$catgstr_urlpass;?>','<?php if($this->uri->segment(4) != ''){ echo $strng;}else{ echo 'NOT';}?>')" >        <option value=''>--Select--</option>
		  ...
		  <div class="grid1_of_4 catlg">
               ...
               <?php /*?> <a href="<?php echo base_url().'product_description/product_detail/'.preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" ><?php */?>
                 ...
                 <a href="<?php echo base_url().preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" >
    				...
    				<?php }else{?>
					<?php /*?><img src="<?php echo base_url();?>images/product_img/<?=$image_arr[0];?>" onerror="imgError(this);" class="img-responsive wow flipInY" data-wow-delay="1s" alt="<?=$rw->name.'_moonboy' ?>"><?php */?>
                    
                    <img src="<?php echo base_url();?>images/product_img/<?=$image_arr[0];?>" onerror="imgError(this);" class="img-responsive" data-wow-delay="1s" alt="<?=$rw->name.'_moonboy' ?>">
   					<?php } ?>
            ...
            <?php } ?>
                    ...
                    <?php /*?><a href="<?php echo base_url().'product_description/product_detail/'.preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" > <?php */?>
					 ...
					 <a href="<?php echo base_url().preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" > 
					<?php if(strlen($rw->name) > 40){ echo substr($rw->name,0,40).'...';}else{ echo $rw->name;}?></a> </h2>
		...
		<?php /*?><div class="modal fade" id="myModal<?=$sl?>" role="dialog" aria-labelledby="myModalLabel"> 
          ...
          <?php } // End of product data is not false condition ?>
           ...
           <div id="view_more_dv">
				...
				<input type="button" id="view_moreproduct" class="add-to-cart view_mor" value="View More" name="button" onClick="ShowMoreData('<?=$no_of_product;?>','<?=$catgstr_urlpass;?>','<?php if($this->uri->segment(4) != ''){ echo $this->uri->segment(4);}else{ echo 'NOT';}?>')">
				<?php }	?>
           </div>
            <br>
