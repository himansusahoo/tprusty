...
function filter_attribute(cat_name,cat_id,lastseg,attrbtype,attrbvalue,attrbsli){
		...
		window.onload = $('#limg').css('display','block');
		if(lastseg == 'NOT'){
			window.location.href='<?php echo base_url();?>category/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+attrbtype+'='+attrbvalue;
		}else{ 
				...
				{
					window.location.href='<?php echo base_url();?>category/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+lastseg;
				}
			}
			else{
			window.location.href='<?php echo base_url();?>category/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+lastseg+'&'+attrbtype+'='+attrbvalue;}
		}
          ...
          <h4>Price</h4>
              <div class="row1 scroll-pane">
                  <div class="col col-4">
                    ...
                    <input class="btn-go hvr-sweep-to-right" type="button" value="Search" onClick="FilterProduct_PriceData('<?=preg_replace('#/#',"-",str_replace("'",'-',str_replace('&','-',str_replace(' ','-',strtolower($this->uri->segment(2))))));?>','<?=$label_id;?>','<?php if($this->uri->segment(4) != ''){ echo $strng;}else{ echo 'NOT';}?>')">
		...
		<!-- Type filtering section Start-->
           ...
           <div class="f_sidebar">           
               ...
               <section class="filter-form">
				...
				<?php 
				if($this->uri->segment(3)!='' || $this->uri->segment(3)!='NOT')
				{if(strpos(str_replace('%20',' ',$this->uri->segment(3)),$valattrbhead)==false) 
					{
						echo 	"collapse";
					}
				}
               ...
               <li title="<?php echo trim($res_attrbval['attr_value'])?>">
                ...
                <i></i>
				 ...
				 ?>
                       ...
                       <input type="checkbox" id="attrb_realvalue<?=$iattrb?>" name="attrb_realvalue[]" value="<?=trim($res_attrbval['attr_value'])?>" onChange="filter_attribute('<?=preg_replace('#/#',"-",str_replace("'",'-',str_replace('&','-',str_replace(' ','-',strtolower($this->uri->segment(2))))));?>','<?=$label_id;?>','<?php if($this->uri->segment(4) != ''){ echo $strng;}else{ echo 'NOT';}?>','<?=trim($res_attrbval['attr_id']).'-'.trim($valattrbhead);?>','<?=trim($res_attrbval['attr_value'])?>','<?=$iattrb?>')" 
...
<a href="<?php echo base_url() ?>" style="color:#666;">Home </a> > <a href="<?php echo base_url().'category/'.$label_name ?>"><?=$bredcum_name?> </a>
<!-- Breedcum end-->
...
<div class="w_content">
		<div class="catlog">
        	...
        	<div>
				...
				<?php if(@$arr1[1]=='Low-To-High'||  @$arr1[1]=='High-To-Low')   { echo " ";}else{ echo "  "?><span class="rst_spn" onClick="resetFilterIndivisually('<?=preg_replace('#/#',"-",str_replace("'",'-',str_replace('&','-',str_replace(' ','-',strtolower($this->uri->segment(2))))));?>','<?=$label_id;?>','<?=$this->uri->segment(4);?>','<?php echo $arr1[0].'='.$arr1[1];?>')"><?php echo str_replace('%20',' ',$arr1[1]);?>&nbsp; <i class="fa fa-times close_filter" aria-hidden="true" ></i></span>&nbsp;&nbsp;&nbsp;&nbsp;
<?php }  ?>
                ...
                <div class="dropdown_left">
			       Sort By Price &nbsp;<select class="dropdown selectpicker" id="attr_size"  data-style="btn-info" style="width:auto;" onChange="sortby_price(this.value,'<?=preg_replace('#/#',"-",str_replace("'",'-',str_replace('&','-',str_replace(' ','-',strtolower($this->uri->segment(2))))));?>','<?=$label_id;?>','<?php if($this->uri->segment(4) != ''){ echo $strng;}else{ echo 'NOT';}?>')" >        <option value=''>--Select--</option>
		...
		<?php /*?><div class="modal fade" id="myModal<?=$sl?>" role="dialog" aria-labelledby="myModalLabel"> 
          ...
          <?php } // End of product data is not false condition ?>
           ...
           <div id="view_more_dv">
				...
				<input type="button" class="add-to-cart view_mor" value="view more" name="button" onClick="ShowMoreData('<?=$no_of_product;?>','<?=$label_id?>','<?php if($this->uri->segment(4) != ''){ echo $this->uri->segment(4);}else{ echo 'NOT';}?>')">
				<?php }	?>
           </div>
</div>
</div>
