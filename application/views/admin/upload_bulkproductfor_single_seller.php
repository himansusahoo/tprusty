...
function select_attrbset(catg_id)
{
	...
	$('#attrb_ascatgwise').text('');
	$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Bulkupload_singleslr/select_catgid",
...
function dwln_bulkprodtemplate(selr_id)
{
 ...
 <div class="row content-header">
             ...
             <?php if($_SERVER['HTTP_REFERER'] !=base_url()."admin/sellers/sellerprofile_details/".$this->uri->segment(4).""){ ?>   
...
<?php if(in_array($category->category_id,$prodattg_arr)) { ?><?php */?>
                                 ...
                                 <tr>
									...
									<td>
                                    	<?php /*?><select class="seller_input" id="attribute_set" name="attribute_set" onchange="select_attrbset(this.value)">
                                                    ...
                                                    <span id="attrb_ascatgwise"> </span></td>
                                                    <td><button id="product_submit" class="seller_buttons" onClick="dwln_bulkprodtemplate('<?=$seller_id?>')" >
