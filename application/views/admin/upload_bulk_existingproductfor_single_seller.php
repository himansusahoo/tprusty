...
function select_attrbset(catg_id)
{
	...
	//$('#hidden_subcatgtxtbox').text() = catg_id;
...
/*$(document).ready(function(){
 ...
 <div class="row content-header" style="background-color:#CCC;">
				<h3 style="margin-top:0px; font-weight:bold;"><span style="color:#F00">Upload Bulk Existing Products</span></h3>
                 ...
                 <a id="product_uploadlog" class='seller_buttons' href="<?php echo base_url().'admin/Upload_bulk_existingproduct/uploadlog_extingproductlist/'.$seller_id ?>" style="cursor:pointer;" >
           				<i class="fa fa-list-alt" aria-hidden="true" style="color:#FFF;"></i> &nbsp;Upload Log 
           		</a>
            </span>  
                <br><br>
                <div class="exist_prod_new_list" align="right"><button type="button" onclick="window.location.href='<?php echo base_url(). 'admin/Upload_bulk_existingproduct/bulk_existingproductupload_panel/'.$seller_id?>' " style='width:225px;'>Bulk Existing Products Upload &nbsp;<i class="fa fa-upload" aria-hidden="true"></i>
</button>  </div>
...
echo form_open('admin/Upload_bulk_existingproduct/select_productfor_addexistingproduct',$formarr); ?> 
...
foreach($categories as $category){ ?>
      ...
      <?php $qr=$this->db->query("select * from category_indexing where parent_id='$category->category_id' "); 
       ...
       <li>		
			<input type="radio" id="subcategory_id" name="subcategory_id" value="<?php echo $rs1->category_id; ?>" onClick="select_attrbset('<?php echo $rs1->category_id; ?>')"/>
