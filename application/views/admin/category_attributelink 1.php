...
<form action="<?php echo base_url().'admin/super_admin/add_attrilink' ?>" method="POST">
        ...
        <tr>
            ...
            <td>
                 <?php $qr=$this->db->query("SELECT DISTINCT lvl2, lvl2_name, lvl1, lvl1_name , lvlmain_name
													FROM  `temp_category` 
													WHERE lvl1 !=0 ");
											$rw=$qr->result();
											?>
                    ...
                    <input  type="reset" class="all_buttons" id="new_courier_btn" value="Reset" >
                      ...
                      <div id="loader_div" style="display:none; text-align:center;"> <img src="<?php echo base_url().'images/loading1.gif' ?>" /> 
						...
						<?php	}?> 
                            ...
                            <a class='inline' href="#inline_content_edit_cour_fld" onClick="edit_wtchrgs('<?//=$cate_attribute['attribute_group_id']?>','<?//= $cate_attribute['attribute_group_name']?>','<?//= $cate_attribute['cate_attributelink']?>')" title="Edit"><i class="fa fa-pencil-square-o" style="font-size:16px;"></i></a>-->&nbsp;&nbsp;&nbsp;&nbsp;	<span onClick="remove_ctag('<?=$cate_attribute['attribute_group_id']?>','<?= $cate_attribute['attribute_group_name']?>')" title="Remove All Category Link" style="cursor:pointer; color:#F00;">Delete All</span>
...
<div class="col-md-12">
<form action="<?php echo base_url().'admin/super_admin/edit_attrilink' ?>" method="POST">
		<table class="edit_address_form">
         <input type="hidden" name="attri_groupid" value="<?=$catg_row->attribute_group_id;?>"/>
         <tr>
                <td>Attribute Group Name: </td>
                <td><select name="catg_name" id="catg_name" data-placeholder="Choose Category" class="text2">
                                            <?php $query=$this->db->query("SELECT * FROM attribute_group ORDER BY ASC");
											$attrgrnm=$query->row();
											?>
                                            <?php if($attrgrnm){ ?>
                                            <option value="<?=$attrgrnm->attribute_group_id;?>"><?=$attrgrnm->attribute_group_name;?></option>
            ...
            <td>
                ...
                <select name="attrilinkgroup[]" id="attrilink" data-placeholder="Choose Category" class="chosen-select" multiple tabindex="4">
                
                                            <?php $qr=$this->db->query("
