...
function remove_menulink(pgsection_sqlid,menu_id)
{
	...
	if(conf){
		$('#loader_div').css('display','block');
			$.ajax({
				method:"POST",
				url:"<?php echo base_url().'admin/Desktop_page_catlog/remove_categorymenulink' ?>",
			...
			<div id="content">  
                ...
                <?php require_once('pagedesign_setup/desktop_catlog_addsectionjs.php'); ?>
                	<?php
					//$attributes = array('onSubmit' => 'return valid_columndata()');
					echo form_open_multipart('admin/Desktop_page_catlog/update_catlogectiondata');
					?>
                	<!--<form onSubmit="return membershipInfo()">-->
					<div class="row content-header">
						<div class="col-md-8"><h4><b>Edit Section Of Catalog Page</b></h4><div id="ssmessg"><?= $this->session->flashdata('succss_msg'); ?></div></div>
						<div class="col-md-4 show_report">
                         <button type="button" id="section_submit" class='all_buttons' style="background-color:#C03;float:right; font-weight:bold; height:32px; width:72px;" onClick="window.location.href='<?php echo base_url(); ?>admin/Desktop_page_catlog/<?php if($this->uri->segment(2)=='Desktop_page_catlog'){ echo 'class="selected"';} ?>'" >
					...
					<div class="form_view">
						<h3>Edit Section Of Catalog Page</h3>
							<table>
                            	<tr>                            
                            <td colspan="2">
                                ...
                                <div style="width:100%; height:150px; overflow-y:scroll;">
                                    <table class="table table-bordered">
                                            ...
                                            <tr bgcolor="#99CCFF">
                                                ...
                                                <th width="30%">Third Level Menu Link</th>						
										   ...
										   ?>
                                            <tr>
                                                ...
                                                <?php
													...
													$qr_child=$this->db->query("SELECT label_name,parent_id FROM category_menu_desktop WHERE dskmenu_lbl_id='$vsal'");
													
													$child_menulabel=$qr_child->row()->label_name;
													$parent_id=$qr_child->row()->parent_id;
													
													$qr_parent=$this->db->query("SELECT parent_id,label_name FROM category_menu_desktop WHERE dskmenu_lbl_id='$parent_id'");
													...
													category_menu_desktop WHERE dskmenu_lbl_id='$parent_dsklblbid' ");
													$mainparent_lblname=$qr_mainparent->row()->label_name;
													
													echo $mainparent_lblname.'>>'.$parent_menulabelname.' >> '.$child_menulabel;
												?>
                            ...
                            <?php if($sec_info->row()->sec_type!='Product Catalog') {?>
                            <tr>
                                    <td> Third Level Menu Link</td>
                                    <td>
											...
											<?php
                                                 if($sel_catg->num_rows()>0)
                                                   ...
                                                   $dskmenu_lbl_id=$rw['parent_id'];
													...
													category_menu_desktop WHERE dskmenu_lbl_id='$dskmenu_lbl_id' ");
                             ...
                             <?php } ?>   
                                ...
                                <?php
									...
									$img_size=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='desktop' AND page_nm='catalog' AND culumns_count='$clmn_conunt' ");?>
                                ...
                                <tr>
								   ...
								   $qr=$this->db->query("SELECT ".$sec_type." as srcshot_imgname FROM pagedesign_imagesize WHERE display_type='desktop' AND page_nm='catalog' AND culumns_count='$col_num' AND img_size='$img_size' ");								  
								   ?>
                                   <?php if($sec_info->row()->sec_type=='Prodcts Vertical section') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_catlog/product_vertical_section.png'; ?>" >									
                                   <?php } ?>
                                    <?php if($sec_info->row()->sec_type=='Video') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_catlog/video.png'; ?>" >								
                                   <?php } ?>
                                   
                                    <?php if($sec_info->row()->sec_type=='Product') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_catlog/product_product_clmn2_300x400.png'; ?>" >								
                                   <?php } ?>
                                   
                                   <?php if($sec_info->row()->sec_type=='Rich Text Editor') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_catlog/Rich_texteditor.png'; ?>" >								
                                   <?php } ?>
                                   ...
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_catlog/'.$qr->row()->srcshot_imgname; ?>" >									<?php } ?>
                            ...
                            <div id="columninfo_div" style="background-color:#099;display:<?php if($sec_info->row()->sec_type!='Rich Text Editor' && $sec_info->row()->sec_type!='Product Catalog') {echo "block";}else{echo "none";}?>;" align="center">
                           <?php 
						   $i=1;
						   $sec_idclmn=$sec_info->row()->Sec_id;
						   $qr_clmnid=$this->db->query("SELECT * FROM desktopsite_columninfo WHERE page_id='3' AND sec_id='$sec_idclmn'  ");
						   ...
						   if($qr_clmnid->num_rows()>0){
                                ...
                                <fieldset id="imgelbl<?=$i.$j?>"><legend style="background-color:#CCC; font-size:13px; font-weight:bold;">Image Label</legend><span class="textarea_val<?=$i.$j?>" style="width:20%; float:right; margin-top: -52px; padding: 16px; display: inline-block;"></span><textarea name="imglab_txtara<?=$i?>[]" id="imglab_txtara<?=$i.$j?>" class="imglab_txtara<?=$i.$j?>" onKeyUp="onkeuup(<?=$i.$j?>)" maxlength="6"  style="width:127px;" placeholder="Enter Image Label"><?=stripslashes($res_imgdata['imag_label'])?></textarea></fieldset>
                                </td>
                                <td>
								...
								<fieldset  style="display:<?php if($res_imgdata['sku']=='') {echo "none";}?>;" id="sortbyinfo<?=$i.$j?>">
									<legend style="background-color:#CCC; font-size:13px; font-weight:bold;">Sort By</legend>
...
function remove_img(i,j,img_sqlid)
{
	...
	$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_catlog/remove_imagedata",
...
function remove_csvfile(i,j,img_sqlid,cvsfilenm)
{
	...
	if(conf)
	{
		...
		$.ajax({
			method:"POST",
			url:"<?php echo base_url(); ?>admin/Desktop_page_catlog/remove_csvdata",
...
function onkeuup(valuid){
	...
	var text_remaining = text_max - text_length;
	 $('.textarea_val'+valuid).html('6/'+text_remaining);
	}
</script>
