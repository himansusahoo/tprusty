...
<script language="JavaScript">
    ...
    function change_order_status()
    {
        ...
        if ($('#order_status').val() == "")
        {
            alert("select change status value");
            return false;
        }
                        ...
                        foreach ($order_list->result() as $rws) {
                            ...
                            <tr <?php if (in_array($rws->order_id, $transfer_orderno) == true) {
                                ...
                                <td>
                                    ...
                                    <?php
                                    echo '<br>';
                                    if ($rws->payment_mode == '1' && $rws->order_processstatus == 'Order Placed Successfully By Buyer') {
                                        ?>
                                ...
                                <td><?php
                                    $dt_order = substr($rws->date_of_order, 0, 10);
                                    echo date('d-M-Y', strtotime($dt_order));
                                    ?></td>
                                <td><?php
                                    $dt_orderstatus = substr($rws->order_status_modified_date, 0, 10);
                                ...
                                <td>
                                    ...
                                    $qrs1 = $this->db->query("select * from shipment_info where order_id='$rws->order_id'");
                                    $cts1 = $qrs1->num_rows();
                                    if ($cts1 == 0) {
                                        
                                    } else {
                                        
                                    } if ($rws->order_confirm_for_seller == 'Not Approved') {
                                        ?>   
