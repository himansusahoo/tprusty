			 ...
			 <!-- @end top-bar  -->
                ...
                <?php if($this->session->flashdata('flshmsg')){ ?>	<div id="successfully_verify" style="color:#0C0;" align="center">
                    ...
                    <div class="col-md-10 mt20">
						<?php /*?><button id="product_submit" class='seller_buttons' style="background-color:#666;float:right;" onclick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/addnewsection_fordesktophomepage'; ?>'" > 
						...
						<img src="<?php //echo base_url().'images/success_icon.png' ?>">&nbsp<?php //echo $this->session->flashdata('msgcod_wtchrg'); ?></div> <?php //} ?>		
                            <a id="product_submit" class='seller_buttons add-new-sec' style="background-color:#666;float:right;" href="<?php echo base_url().'admin/desktop_page_setup/addnewsection_fordesktophomepage'; ?>" target="_blank">
                            ...
                            <!--Data Display from page section table start-->
								...
								?>
                                   ...
                                   <?php } ?>
                                    ...
                                    <td>
                                    <?php if($sort_i>1) {?>
                                    <img src='<?php echo base_url().'images/icon_finder/1495193298_Stock Index Up.ico' ?>' style="width:25px;height:29px;cursor:pointer;" onClick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/hompagesortby_section_up/'.$res_secinfo['pgsection_sqlid'] ?>' " > &nbsp;  
									
									<span><a style="cursor: pointer; color:#333" onClick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/hompagesortby_section_up/'.$res_secinfo['pgsection_sqlid'] ?>' ">Up </a></span>
									<span style="float:right;"><a class="fa fa-angle-double-up" style="cursor: pointer; color:#333; font-size:20px;" title="Move To Top" onClick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/hompagesortby_section_movetop/'.$res_secinfo['pgsection_sqlid'] ?>' "></a></span>
									
									<?php } ?><br>
                                    <?php if($sort_i!=$sec_ctr){  ?>
                                    <img src='<?php echo base_url().'images/icon_finder/1495193373_Stock Index Down.ico' ?>' style="width:25px;height:29px;cursor:pointer;" onClick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/hompagesortby_section_down/'.$res_secinfo['pgsection_sqlid'] ?>' " >
									
									<span><a style="cursor: pointer; color:#333" onClick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/hompagesortby_section_down/'.$res_secinfo['pgsection_sqlid'] ?>' ">Down</a></span>&nbsp;&nbsp;
									<span style="float:right;"><a class="fa fa-angle-double-down" style="cursor: pointer; color:#333; font-size:20px;" title="Move To Buttom" onClick="window.location.href='<?php echo base_url().'admin/desktop_page_setup/hompagesortby_section_movedown/'.$res_secinfo['pgsection_sqlid'] ?>' "> </a></span>
                                    <?php } ?>
                                    </td>
                                    <td>
										<a href="<?php echo base_url().'admin/desktop_page_setup/edit_desktophomepagesection/'.$res_secinfo['pgsection_sqlid'] ?>" target="_blank">
                                        <img src='<?php echo base_url().'images/icon_finder/1495194794_edit-notes.ico' ?>' style="width:33px;height:29px;cursor:pointer;" >
										&nbsp;</a>
										<span><a style="cursor: pointer; color:#333;" href="<?php echo base_url().'admin/desktop_page_setup/edit_desktophomepagesection/'.$res_secinfo['pgsection_sqlid'] ?>" target="_blank">Edit </a></span><br>
...
function valid_remove(sec_sqlid)
{
	...
	if(conf)
	{
		window.location.href='<?php echo base_url().'admin/desktop_page_setup/remove_desktophomepagesection/'?>'+sec_sqlid ;
	}
}
	...
	if($('#sec_status').val()=="")
		...
		else
		{
			...
			$('#loader_div').css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/desktop_page_setup/change_sec_status",
