...
<script src="<?php echo base_url();?>js/img_uplod_script/jquery.uploadfile11.min.js"></script>

<script>
$(document).ready(function(){
	for(var sl = 1; sl<3; sl++){
		$("#uploadfile"+sl).uploadFile({
			url: "<?php echo base_url();?>admin/configuration/upload_tmp_box1_image",
			dragDrop: true,
			fileName: "userfile",
			returnType: "json",
			showDelete: true,
			...
			deleteCallback: function (data, pd) { 
				//for (var i = 0; i < data.length; i++) {
					$.post("<?php echo base_url();?>admin/configuration/delete_tmp_box1_image", {op: "delete",name: data},
		...
		<div id="content">    
          ...
          <!-- <button type="submit" class="all_buttons">update</button>-->
            </div><h3>Block1 Setting</h3></div>
            
            <div>
             <h4>Box1 Image</h4>Minimum Image Size to be Uploaded is 595*300
                        ...
                        <tr class="table_th">
                            ...
                            <th width="20%"></th>
                             ...
                             $rows1 = $image_info->num_rows();
                              if($rows1 > 0){
								  $sl=0;
                                    foreach($image_info->result() as $rows){ 
									$sl++;
									$arr_img = explode(',', $rows->image_name);
                                ?>
                        <tr>
                        	<td><?=$rows->category_name?></td>
                            <td>  
                                <img src="<?php echo base_url().'images/subpage/'.$rows->image_name; ?>" class="list_img" id="image_edit">
                            </td>
                                            	...
                                            	<div class="form_view">
                                                	<h3>Update Box Image</h3>
                                                        ...
                                                        <table style="padding:10px;">
                                                            <tr>
                                                                <td>Image URL : </td>
                                                                <td><?php echo base_url()."images/subpage/".$rows->image_name;?></td>
                                                            </tr>
                                                            <tr>
                                                                <td>Category Name : </td>
                                                                <td><?=$rows->category_name?></td>
...
<?php
   echo form_open_multipart('admin/configuration/update_box1_image');
?>		
														<table>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <input type="hidden" id="slider_id" class="slider_id" name="hidden_slider_id" value="<?=$rows->image_id?>">
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                            ...
                                                            <tr>
                                                                <td colspan="2" class="a-center"><button type="submit" class="seller_buttons" onClick="return validate_box1_image_form(<?=$sl;?>)">Update</button></td>
                                                            </tr>
                                                        </table>
                                                    </form>
                                                </div>
...
window.location.href="<?php echo base_url()?>admin/configuration/box1_delete/"+imageid+"/"+imagename;
 ...
 <script>
	function validate_box1_image_form(sl){
