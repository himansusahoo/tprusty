...
<script src="<?php echo base_url();?>js/img_uplod_script/jquery.uploadfile12.min.js"></script>

<script>
$(document).ready(function(){
	for(var sl=1; sl<4; sl++){
		$("#uploadfile"+sl).uploadFile({
			url: "<?php echo base_url();?>admin/configuration/upload_tmp_box2_image",
			dragDrop: true,
			fileName: "userfile",
			returnType: "json",
			showDelete: true,
			...
			deleteCallback: function (data, pd) { 
				//for (var i = 0; i < data.length; i++) {
					$.post("<?php echo base_url();?>admin/configuration/delete_tmp_box2_image", {op: "delete",name: data},
		...
		<div id="content">    
          ...
          <!-- <button type="submit" class="all_buttons">update</button>-->
            </div><h3>Block2 Setting</h3></div>
            
            <div>
             <h4>Box1 Image</h4>Minimum Image Size to be Uploaded is 357*291
                        ...
                        <tr class="table_th">
                         ...
                         <?php
						 $rows1 = $box1select->num_rows();
						  if($rows1 > 0){
							  $sl=0;
							foreach($box1select->result() as $row){
								$sl++;
						?>
                        <tr>
							<td><?=$row->category_name?></td>
							<td>   
                                <img src="<?php echo base_url().'images/subpage2/'.$row->image_name; ?>" class="list_img"> &nbsp; &nbsp;&nbsp;&nbsp;
                            </td>
...
<?php
   echo form_open_multipart('admin/configuration/update_box2_image');
?>		
														<table>
                                                            <tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <select id="category_id<?=$sl;?>" class="text2" name="categoryid2">
                                                            ...
                                                            <tr>
                                                                <td colspan="2" class="a-center"><button type="submit" class="seller_buttons" onClick="return validate_box2_image_form(<?=$sl;?>)">Update</button></td>
                                                            </tr>
                                                        </table>
                                                    </form>
                                                </div>
...
<script>
function validate_box2_image_form(sl){
