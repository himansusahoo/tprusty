 ...
 <div class="row content-header" style="background-color:#CCC;">
		...
		<!-------------------------------------------Listing start--------------------------------------------------------------------->
                     ...
                     <div class="col-md-6 right">
					  ...
					  <div><form action="<?php echo base_url().'admin/Upload_bulk_existingproduct/filter_bulkexisting_product/'.$catg_id.'/'.$attrbsetid.'/'.$seller_id.'/' ?>" method="get" onsubmit="return validetdate()" >
							...
							<tr class="table_th">
                             ...
                             <td>
                              <!--<input type="text" class="seller_input" name="from_dtm1" id="datepicker-example7-start" style="width:300px">-->
                               ...
                               From Date:     <div class="form-group">
                          ...
                          <div class="form-group">
                            <div class='input-group date' id='datetimepicker7'>
                             ...
                             <td> 
                             	...
                             	<input type="text" class="seller_input" name="seller_name" id="seller_name" style="width:280px">
    ...
    <!--------------------------------------------Form submit for excel template genarate start-------------------------------------------->                    
						...
						echo form_open('admin/Bulkproduct_edit/download_extprodtemplate',$from_arr); 
           ...
           <i class="fa fa-download" aria-hidden="true" style="color:#FFF;"></i> &nbsp;Download Products 
             ...
             <div class="exist_prod_new_list" align="left"><button type="button" onclick="window.location.href='<?php echo base_url(). 'admin/Upload_bulk_existingproduct/bulk_existingproductupload_panel/'.$seller_id?>' " style='width:225px;'>Bulk Existing Products Upload &nbsp;<i class="fa fa-upload" aria-hidden="true"></i>
</button>  </div>
              <?php } ?>
                        ...
                        <input type="hidden" id="hiddenbox_sellerid" name="hiddenbox_sellerid" value="<?php echo $seller_id; ?>">
							...
							<table class="table table-hover" style="border: 1px solid #c8c8c8;">
								...
								if($search_result->num_rows()>0) { 
									...
									$chk_sku=$row['sku'];
									$qr_extprodchkk=$this->db->query("SELECT sku FROM seller_product_master WHERE sku='$chk_sku' ");
									$countcheck_extprod=$qr_extprodchkk->num_rows();
									
									if(!in_array($row['product_id'],$prodid_arr) && $countcheck_extprod==0){
								...
								<tr id="prod<?=$row['product_id']?>">
									...
									<td>
										<div class="row">
											<div class="col-md-4">
												<input type='checkbox' name="prodid_chk[]"  onclick="add_exist_product('<?=$row['product_id']?>', '<?=$row['seller_id']?>','<?=$row['sku']?>','<?=$sl?>')" id="prodchk<?=$row['product_id']?>" value="<?=$row['product_id']?>">
                                                
                                              <span style="display:none;">  
                                              <input type='checkbox' name="prodskuid_chk[]" id="prodskuid_chk<?=$sl?>"  value="<?=$row['sku']?>">
                                              </span>  
								...
								$sl++;	} // array of product_id check end
...
function add_exist_product(prodid,selrid,sku,sl)
{
