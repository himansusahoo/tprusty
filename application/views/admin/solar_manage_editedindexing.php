    ...
    <div class="main-content">
        ...
        <div class="row content-header">
            ...
            <div style="background-color:  #d7bde2  ; border: 1px solid; border-radius: 5px; height:50px;" ><b style="font-size:24px;"> Indexing Of:&nbsp;</b>
                ...
                <select id="indextype_slct" name="indextype_slct" class="text2" style="width:200px; height:30px;" onChange="indextypeslct(this.value)">
                    ...
                    if ($this->uri->segment(3) == 'Solar_manage') {
                        echo "selected";
                    }
                    ?>>New Product</option>
                    <option value="Edit" <?php
                    if ($this->uri->segment(3) == 'edited_productindex') {
                        echo "selected";
                    }
                    ?>>Edited Product</option>  
                    <option value="Delete" <?php
                            if ($this->uri->segment(3) == 'removed_productindex') {
                                echo "selected";
                            }
                    ?>>Deleted Product</option>                                  
                </select>
            </div>
            ...
            <h4><img src='<?php echo base_url() . 'images/Solr_Logo_on_white.png' ?>' style="width:70px;height:40px;"> Indexing Of Edited Product</h4>
        </div>
        ...
        <div class="col-md-6 left">
            ...
            <table>
                <tr>
                    <td></td>
                    <td><button type="button" style="background-color:#dc7633;"  class="seller_buttons" onClick="valid_editedslctsku()" id="selctedproductindex_btn"  ><i class="fa fa-list-alt" aria-hidden="true"></i>&nbsp;Update Indexing On Selected</button></td>
<!--<td>
<div id="img_info"></div>
...
<?php $attrb = array('method' => 'GET');
echo form_open('admin/Solar_manage/edited_productindex', $attrb);
?>  
                    <tr class="filter_tr">
                        <td> </td>
                        <td>
                        ...
                        <td>
                            ...
                            <button type="button" onClick="window.location.href = '<?php echo base_url() . 'admin/Solar_manage/edited_productindex' ?>'" 
                ...
                <?php } else { ?>
                                ...
                                <?php
                                if ($row['error_msg'] != '') {
                                    echo "<br>[Error:<span style='color:#F00'> " . $row['error_msg'] . "</span> ]";
                                }
                                ?>
                                    </td>
...
<script>
    ...
    function valid_editedslctsku()
    {
        ...
        } else
        {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() . 'admin/Solar_manage/update_editedprodsolrindex' ?>",
                data: {prod_sku: prod_sku},
                success: function (data) {
