...
//		{  
		var menu_lable_id=$('input[name=subcategory_id]:radio:checked').val();			
			...
			window.location.href="<?php echo base_url().'admin/Super_admin/edit_mobilemenu/' ?>"+menu_lable_id;	
		//}
	...
	function valid_menulabeliddelete()
	{
			...
			if(conf)
			{
				...
				window.location.href="<?php echo base_url().'admin/Super_admin/delete_mobilemenu/' ?>"+menu_lable_id;	
			}
	}
...
function remove_ctag(dskmenu_lbl_id,catg_id)
{
	...
	$('#loader_div').css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url().'admin/super_admin/remove_mobilecategorylink' ?>",
...
<div id="content">  
    ...
    <div class="main-content">
		...
		<?php if(@$data==true){ ?><div align="center" style="color:#0C6"> <?php  echo $data ?> </div> <?php } ?>
          ...
          <form id="save_catgory_form" method="post" action="<?php echo base_url().'admin/Super_admin/update_mobilemenusetup' ?>" enctype="multipart/form-data">
           ...
           <div class="col-md-4 show_report">
               ...
               <?php /*?> <input  type="button" class="all_buttons" value="Add Subcategory" onClick="window.location.href='<?php echo base_url().'admin/catalog/add_subcategory' ?>' " /><?php */?>
				...
				<input type="button" class="all_buttons" value="Reset" onClick="window.location.href='<?php echo base_url().'admin/super_admin/mobilemenu_setup'; ?>' " />
  ...
  <ul id="example">
  <?php
  	$thirdrd_lvlmenuid=array();
   foreach($data2->result() as $rows){ ?> <!--level-1 --> 
    <li id="category_li">
      ...
      <ul >
      <?php $qr=$this->db->query("select * from category_menu_mobile where parent_id='$rows->dskmenu_lbl_id' order by order_by "); 
	  	//$rw=$qr->result();
	  	$ct=$qr->num_rows();
       ...
       <li>		
         ...
         <ul>
         <!--level-3-->
          <?php $qr1=$this->db->query("select * from category_menu_mobile where parent_id='$rs->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct1=$qr1->num_rows();
        ...
        <?php 
			foreach($qr1->result() as $rs1){ $thirdrd_lvlmenuid[]=$rs1->dskmenu_lbl_id;?>
       <li>		
         ...
         <ul>
         <!--level-4-->
          <?php $qr2=$this->db->query("select * from category_menu_mobile where parent_id='$rs1->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct2=$qr2->num_rows();
       ...
       <li>		
         ...
         <ul>
         <!--level-5-->
          <?php $qr3=$this->db->query("select * from category_menu_mobile where parent_id='$rs2->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct3=$qr3->num_rows();
       ...
       <li>		
         ...
         <ul>
         <!--level-6-->
          <?php $qr4=$this->db->query("select * from category_menu_mobile where parent_id='$rs3->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct4=$qr4->num_rows();
       ...
       <li>		
		 ...
		 <?php echo	$rs4->label_name;?>
          ...
          <ul>
         <!--level-7-->
          <?php $qr5=$this->db->query("select * from category_menu_mobile where parent_id='$rs4->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct5=$qr5->num_rows();
       ...
       <li>		
         ...
         <ul>
         <!--level-8-->
          <?php $qr6=$this->db->query("select * from category_menu_mobile where parent_id='$rs5->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct6=$qr6->num_rows();
       ...
       <li>		
		 ...
		 <?php echo	$rs6->label_name;?>
          ...
          <ul>
         <!--level-9-->
          <?php $qr7=$this->db->query("select * from category_menu_mobile where parent_id='$rs6->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct7=$qr7->num_rows();
       ...
       <li>		
		 ...
		 <?php echo	$rs7->label_name;?>
            ...
            <ul>
         <!--level-10-->
          <?php $qr8=$this->db->query("select * from category_menu_mobile where parent_id='$rs7->dskmenu_lbl_id' order by order_by"); 
	  	//$rw=$qr->result();
	  	$ct8=$qr8->num_rows();
                         ...
                         <input type="button" value="Delete Menu" class="btns" name="delete_mnb" id="delete_mnb" disabled onClick="valid_menulabeliddelete()" /></h3>
							<table>
								<tr>
                                    ...
                                    <input type='hidden' name='hdnextimg_name' id='hdnextimg_name' value='<?php echo $pcmenu_edit->menu_image ?>' >
                                    
                                    <input type="text" class="text2" name="menu_name" id="menu_name" value="<?php echo $pcmenu_edit->label_name ?>"></td>
								</tr>
                                
                                 <tr>
									<td style="width:20%;"> Display URL <sup>*</sup> </td>
                                ...
                                <tr>
                                  ...
                                  <tr>
                                    ...
                                    <option value="0">No Link</option>
                                            <?php
											$mob_catglinkarr=explode(',',$pcmenu_edit->category_id);
											 foreach($category_result as $catg_row){ ?>
                                            <?php if(in_array($catg_row->lvl2,$mob_catglinkarr)){ ?>
			...
			$result_catg= $qr->result();	
                ...
                <?php } ?>
                    ...
                    <!-------------------------Image upload div start -------------------------->
                       		<?php //if(!in_array($pcmenu_edit->dskmenu_lbl_id,$thirdrd_lvlmenuid)) {?>		
                                   <div id="imag_div" align="center">
                                   		
                                        <fieldset style="background-color:#76d7c4; border: 1px solid; border-radius: 10px; text-align:center;">
                                        <legend style="background-color:#e5e7e9;border: 1px solid; border-radius: 25px;">
                                        <?php if($pcmenu_edit->parent_id==0){?>Upload Menu Icon<?php }else{ ?> Upload Menu Image<?php } ?>
                                        
                                        </legend>
                                        	<div align="center" style="color:#F00; font-weight:bold; background-color:#FFF;"> 
											<?php if($pcmenu_edit->parent_id==0){?>Image Size : 145x145 Pixel 
											<?php }else{ ?> Image Size : 150x216 Pixel<?php } ?> 
                                            </div>
                                            
                                        	<?php if($pcmenu_edit->parent_id==0) {?>
                                            <input type="file" id="imgInp" name="userfile" onChange="icon_preview_img(this)">
                                           <?php }else{ ?>
                                           <input type="file" id="imgInp" name="userfile" onChange="preview_img(this)">
                                           <?php } ?>
                                           
                                           <img id="imgprvw" src="#" alt="your image" style="width:100px;height:110px;display:none;" />
                                            <?php if($pcmenu_edit->menu_image!='') {?>
                                            <img  id="ext_imgprvw" src="<?php echo base_url().'images/admin/mobile/mobile_menu/'.$pcmenu_edit->menu_image ?>" alt="your image" style="width:100px;height:110px;" />
                                            
                                            <i id="img_rmv" title="Remove" style="color:#FF0000;cursor:pointer; vertical-align:top" onClick="remove_img('<?=$pcmenu_edit->dskmenu_lbl_id?>','<?=$pcmenu_edit->menu_image?>')" class="fa fa-times" aria-hidden="true"></i>
                                <div id="process_div" style="display:none; color:#090;"> <img src="<?php echo base_url().'images/progress.gif' ?>" />Wait... </div><div id="div_imgsize"></div>
                                            <?php } ?>
                                        </fieldset>		
                                   	
                                   </div> 
                       <?php //} ?>
                   <!-------------------------Image upload div end----------------------------->
...
{ 
	...
	if($.inArray(ext, ['png','jpg','jpeg']) == -1) 
	{
   	...
   	alert('! Invalid Image File. Please Upload Image file with extension .jpg / .jpeg / .png ');
	...
	$('#imgprvw').css('display','block');
 	$('#imgprvw')[0].src = (window.URL ? URL : webkitURL).createObjectURL(input.files[0]);
...
var img = new Image();
 ...
 img.onload = function () {
		...
		valid_imgdimensiion(width,height); //call function
...
function valid_imgdimensiion(width,height) {
    
	var imgdmns_defnd='150x216';	
	var image_dm=width+'x'+height;
	
	if(imgdmns_defnd!=image_dm)
	{	//alert('! Invalid Image. Required Size: '+imgdmns_defnd+' Pixel '  );
		alert("! Invalid Image. Required Size: 150x216 Pixel ");
...
function icon_preview_img(input)
{
...
var img = new Image();
 ...
 img.onload = function () {
		...
		valid_iconimgdimensiion(width,height); //call function
...
function valid_iconimgdimensiion(width,height) {
    
	var imgdmns_defnd='145x145';	
	var image_dm=width+'x'+height;
	
	if(imgdmns_defnd!=image_dm)
	{	//alert('! Invalid Image. Required Size: '+imgdmns_defnd+' Pixel '  );
		alert("! Invalid Image. Required Size: 145x145 Pixel ");
...
function remove_img(menu_sqlid,img_name)
{
	var conf=confirm("Do You Want To Delete Image  ?");
	...
	if(conf)
	{
				...
				$.ajax({
							method:"POST",
							url:"<?php echo base_url(); ?>admin/Super_admin/remov_menuimg",
							data:{menu_sqlid:menu_sqlid,img_name:img_name},
