...
.main{width:80%; margin:10px auto; font-family:"Calibri", Arial, Helvetica, sans-serif;} 
.heading{background-color:#000; font-size:18px; color:#fff; padding:3px 5px; font-weight:bold;}
.main table{margin:10px 0px;  width:100%;}
.main table tr td{font-size:14px;  padding:2px 10px;}
...
<br /><br /><br />
<h1> Invoice </h1>
<?php $check_order_id=$this->db->query("select * from order_transfer where new_order_id='$orderid' ");
...
<?php $qr1=$this->db->query("select * from order_info where order_id='$orderid' "); 
	 ...
	 ?><br />  
    Invoice Number:  <?php $qr1=$this->db->query("select * from order_info where order_id='$orderid' "); 
											$rw1=$qr1->row();
