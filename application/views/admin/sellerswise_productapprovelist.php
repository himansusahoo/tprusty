...
function changeProductStatus(){
	...
	var sel=$("input[name='chk_sellr[]']:checked").val();
	...
	if(approval_status == ''){
		...
		var ys = confirm("Do you want to change seller product status ?");
		if(ys){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>admin/sellers/change_sellerwiseproduct_status_sp",
		...
		<div id="content">
			...
			<div class="main-content">
				<div class="row content-header">
					<h4>Sellerwise New Product Approval</h4>
					<div class="a-center">
					<?php
				...
				<div>
					<table class="table table-bordered">
						...
						<tr class="table_th">
							...
							<th width="5%">Seller Status</th>
                            <th width="5%">Total Pending Products</th>
							<!--<th width="38%">Action</th>-->
						</tr>
						<?php /*?><tr class="filter_tr">
							<td>
								<!--<select>
...
<?php
$qr_prodstscount=$this->db->query("SELECT prod_approv FROM product_process_status WHERE status_id=1 ");
...
?>
						<tr style="color:#FF3737;">
							<td class="a-center">
                            <?php /*?><input type="checkbox" name="seller_id" value="<?=$row->seller_id?>"><?php */?>
                           
                          <?php $rw=$qr_prodstscount->row();
			if($rw->prod_approv=='process')
			{echo " ";
			 //redirect('admin/super_admin/home');
			 }else{ ?>
                            <input type="radio" name="chk_sellr[]" id="chk_sellr<?= $i ?>" value="<?=$row->seller_id ; ?>" >
                    <?php } ?>        
                            </td>
                            ...
                            <td>
                           <?php  $query = $this->db->query("SELECT a.seller_product_id
		...
		WHERE a.product_approve =  'Pending' AND (i.status='Active' OR i.status='Pending') AND a.seller_id='$row->seller_id' group by b.sku,a.seller_product_id ORDER BY a.date_added DESC");
...
?>
						<tr>
							<td class="a-center">
                           <!-- <input type="checkbox" name="seller_id" value="<?//=//$row->seller_id?>">-->
                            <?php $rw=$qr_prodstscount->row();
			if($rw->prod_approv=='process')
			{echo " ";
			 //redirect('admin/super_admin/home');
