			 ...
			 <!-- @end top-bar  -->
               ...
               <?php /*?> <?php if($this->session->flashdata('flshmsg')){ ?>	<div id="successfully_verify" style="color:#0C0;" align="center">
					...
					<div class="row gray_bg">						
						...
						<?php /*?><button id="product_submit" class='seller_buttons' style="background-color:#666;float:right;" onclick="window.location.href='<?php echo base_url().'admin/desktop_page_category/addnewcategory_fordesktophomepage'; ?>'" > 
							<i class="fa fa-plus-square" aria-hidden="true" style="color:#FFF;"></i> &nbsp;Add New Section 
           				</button> <?php */?>
                        
                         <a id="product_submit" class='seller_buttons add-new-sec' style="background-color:#666;float:right;" href="<?php echo base_url().'admin/desktop_page_category/addnewcategory_fordesktophomepage'; ?>" target="_blank">
                        ...
                        <div>
                           <span>
                           		 <?php 
								 $qr_section=$this->db->query("SELECT menu_id FROM desktopsite_pagesectioninfo WHERE page_id='2' AND page_name='category' AND menu_id!=''  ");
...
{ var mnulinkid=$('#menu_link').val();
	...
	else
	{
		$('#loader_div').css('display','block');
			$.ajax({
				method:"POST",
				url:"<?php echo base_url(); ?>admin/Desktop_page_category/populate_pagedesigninfo",
	...
	if(conf)
	{
		...
		var menu_id=$('#menu_link').val();
			...
			$('#process_div'+sec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_category/remove_desktopcategorysection",
	...
	if($('#sec_status').val()=="")
		...
		else
		{
			...
			$('#loader_div').css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_category/change_sec_status",
...
function catlogsortby_section_upajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_category/categorysortby_section_up",
...
function catlogsortby_section_downajx(pgsec_sqlid)
{
	...
	$('#process_div'+pgsec_sqlid).css('display','block');
		$.ajax({
			method:"POST",
			url:"<?php echo base_url(); ?>admin/Desktop_page_category/categorysortby_section_down",
			data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
			success: function (data) {						
...
function catlogsortby_section_movetopajx(pgsec_sqlid)
{
	...
	$('#process_div'+pgsec_sqlid).css('display','block');
		$.ajax({
			method:"POST",
			url:"<?php echo base_url(); ?>admin/Desktop_page_category/categorysortby_section_totop",
			data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
			success: function (data) {						
...
function catlogsortby_section_movedownajx(pgsec_sqlid)
{
	...
	$('#process_div'+pgsec_sqlid).css('display','block');
		$.ajax({
			method:"POST",
			url:"<?php echo base_url(); ?>admin/Desktop_page_category/categorysortby_section_todown",
			data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
			success: function (data) {						
