...
function remove_menulink(pgsection_sqlid,menu_id)
{
	...
	if(conf){
		$('#loader_div').css('display','block');
			$.ajax({
				method:"POST",
				url:"<?php echo base_url().'admin/desktop_page_category/remove_categorymenulink' ?>",
			...
			<div id="content">  
                ...
                <?php require_once('pagedesign_setup/desktop_category_addsectionjs.php'); ?>
                	<?php
					//$attributes = array('onSubmit' => 'return valid_columndata()');
					echo form_open_multipart('admin/desktop_page_category/update_categoryectiondata');
					?>
					...
					<div class="row content-header">
						...
						<div class="col-md-4 show_report">
                         <button type="button" id="section_submit" class='all_buttons' style="background-color:#C03;float:right; font-weight:bold; height:32px; width:72px;" onClick="window.location.href='<?php echo base_url(); ?>admin/Desktop_page_category/<?php if($this->uri->segment(2)=='Desktop_page_category'){ echo 'class="selected"';} ?>'" >
						...
						<h3>Edit Section Of Categories Page</h3>
                            ...
                            <td colspan="2">
										   ...
										   ?>
                                            <tr>
                                                ...
                                                <?php
													...
													$qr_parent=$this->db->query("SELECT label_name FROM category_menu_desktop WHERE dskmenu_lbl_id='$parent_id'");
                                ...
                                <?php
									...
									$img_size=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='desktop' AND page_nm='category' AND culumns_count='$clmn_conunt' ");?>
                            ...
                            <?php } ?>                
                                ...
                                <tr>
                             	...
                             	<div id="div_screenshot" align="center">
								   ...
								   $qr=$this->db->query("SELECT ".$sec_type." as srcshot_imgname FROM pagedesign_imagesize WHERE display_type='desktop' AND page_nm='category' AND culumns_count='$col_num' AND img_size='$img_size' ");								  
								   ?>
                                   <?php if($sec_info->row()->sec_type=='Prodcts Vertical section') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_category/product_vertical_section.png'; ?>" >									
                                   <?php } ?>
                                    <?php if($sec_info->row()->sec_type=='Video') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_category/video.png'; ?>" >								
                                   <?php } ?>
                                   
                                    <?php if($sec_info->row()->sec_type=='Product') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_category/product_product_clmn2_300x400.png'; ?>" >								
                                   <?php } ?>
                                   
                                   <?php if($sec_info->row()->sec_type=='Rich Text Editor') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_category/Rich_texteditor.png'; ?>" >								
                                   <?php } ?>
                                   ...
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/desktop/desktop_category/'.$qr->row()->srcshot_imgname; ?>" >									<?php } ?>
						   ...
						   $qr_clmnid=$this->db->query("SELECT * FROM desktopsite_columninfo WHERE page_id='2' AND sec_id='$sec_idclmn'  ");
						   ...
						   if($qr_clmnid->num_rows()>0){
                                ...
                                <fieldset id="imgelbl<?=$i.$j?>"><legend style="background-color:#CCC; font-size:13px; font-weight:bold;">Image Label</legend><span class="textarea_val<?=$i.$j?>" style="width:20%; float:right; margin-top: -52px; padding: 16px; display: inline-block;"></span><textarea name="imglab_txtara<?=$i?>[]" id="imglab_txtara<?=$i.$j?>" class="imglab_txtara<?=$i.$j?>" onKeyUp="onkeuup(<?=$i.$j?>)" maxlength="5"  style="width:127px;" placeholder="Enter Image Label"><?=stripslashes($res_imgdata['imag_label'])?></textarea></fieldset>
                                </td>
...
function remove_img(i,j,img_sqlid)
{
	...
	$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/desktop_page_category/remove_imagedata",
...
function remove_csvfile(i,j,img_sqlid,cvsfilenm)
{
	...
	if(conf)
	{
		...
		$.ajax({
				method:"POST",
				url:"<?php echo base_url(); ?>admin/desktop_page_category/remove_csvdata",
