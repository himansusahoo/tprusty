			...
			<div id="content">   
			 ...
			 <!-- @end top-bar  -->
               ...
               <?php /*?> <?php if($this->session->flashdata('flshmsg')){ ?>	<div id="successfully_verify" style="color:#0C0;" align="center">
						<img src="<?php echo base_url().'images/success_icon.png' ?>">&nbsp<?php echo $this->session->flashdata('flshmsg'); ?></div> <?php } ?><?php */?>
					<?php
					...
					<div class="row gray_bg">						
						...
						<img src="<?php //echo base_url().'images/success_icon.png' ?>">&nbsp<?php //echo $this->session->flashdata('msgcod_wtchrg'); ?></div> <?php //} ?>									<?php /*?><button id="product_submit" class='seller_buttons' style="background-color:#666;float:right;" onclick="window.location.href='<?php echo base_url().'admin/page_category/addnewcategory_formobilehomepage'; ?>'" > 
							<i class="fa fa-plus-square" aria-hidden="true" style="color:#FFF;"></i> &nbsp;Add New Section 
           				</button> <?php */?>  
                         <a id="product_submit" class='seller_buttons add-new-sec' style="background-color:#666;float:right;" href="<?php echo base_url().'admin/page_category/addnewcategory_formobilehomepage'; ?>" target="_blank">
                        ...
                        <div>
                           <span>
								 ...
								 $qr_section=$this->db->query("SELECT menu_id FROM 
								                       mobilesite_pagesectioninfo WHERE page_id='2' AND page_name='category' AND menu_id!=''  ");
								...
								if($qr_section->num_rows()>0)
								{
									foreach($qr_section->result_array() as $res_menuids)
									{
                                        ...
                                        <select name="menu_link" data-placeholder="Choose Category Links"  
											...
											<option value=''>Select second Level Menu Link</option>
												<?php
						...
						<select id="sec_status" name="sec_status" class="text2" style="width:200px; height:30px;" >
                            ...
                            <!--Data Display from page section table start-->                            	
                                	<tr id=tblrow<?=$res_secinfo['pgsection_sqlid']?>> 
                                        ...
                                        <td> 
                                   <?php
								   if($res_secinfo['sec_type']!='Catagory' && $res_secinfo['sec_type']!='Popular Product' && $res_secinfo['sec_type']!='Category Product'){	     
                                   ...
                                   <?php } ?>
                                    	...
                                    	<td><?php if($sort_i>1) {?>
                                    		<img src='<?php echo base_url().'images/icon_finder/1495193298_Stock Index Up.ico' ?>' style="width:25px;height:29px;cursor:pointer;" onClick="window.location.href='<?php echo base_url().'admin/page_category/categorysortby_section_up/'.$res_secinfo['pgsection_sqlid'] ?>' " > &nbsp;  <?php } ?>
                                    		<?php if($sort_i!=$sec_ctr){  ?>
                                    		<img src='<?php echo base_url().'images/icon_finder/1495193373_Stock Index Down.ico' ?>' style="width:25px;height:29px;cursor:pointer;" onClick="window.location.href='<?php echo base_url().'admin/page_category/categorysortby_section_down/'.$res_secinfo['pgsection_sqlid'] ?>' " >
                                    		<?php } ?>
                                    	</td>
										<td><img src='<?php echo base_url().'images/icon_finder/1495194794_edit-notes.ico' ?>' style="width:33px;height:29px;cursor:pointer;" onClick="window.location.href='<?php echo base_url().'admin/page_category/edit_mobilecategorysection/'.$res_secinfo['pgsection_sqlid'] ?>' " >
                            &nbsp;
...
{ var mnulinkid=$('#menu_link').val();
	...
	else
	{
		$('#loader_div').css('display','block');
					$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/Page_category/populate_pagedesigninfo",
...
function valid_remove(sec_sqlid)
{
	...
	if(conf)
	{
		//window.location.href='<?php //echo base_url().'admin/Page_category/remove_mobilecategorysection/'?>'+sec_sqlid ;
		...
		var menu_id=$('#menu_link').val();
			...
			$('#process_div'+sec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/remove_mobilecategorysection",
	...
	if($('#sec_status').val()=="")
		...
		else
		{
			...
			$('#loader_div').css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/change_sec_status",
...
function catlogsortby_section_upajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/categorysortby_section_up",
...
function catlogsortby_section_downajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/categorysortby_section_down",
					data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
					success: function (data) {						
...
function catlogsortby_section_movetopajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/categorysortby_section_totop",
					data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
					success: function (data) {						
...
function catlogsortby_section_movedownajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/categorysortby_section_todown",
					data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
					success: function (data) {						
