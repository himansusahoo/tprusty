...
<script type="text/javascript" src="<?php echo base_url().'asset/ckeditor/ckeditor.js' ?>"></script>
			...
			<div id="content">  
                ...
                <?php require_once('pagedesign_setup/mobile_searchpage_addsectionjs.php'); ?>
					...
					echo form_open_multipart('admin/Page_search/update_pagesectiondata');
					?>
                	<!--<form onSubmit="return membershipInfo()">-->
					<div class="row content-header">
						<div class="col-md-8"><h4><b>Edit Section Of Mobile Search Page</b></h4><div id="ssmessg"><?= $this->session->flashdata('succss_msg'); ?></div></div>
						<div class="col-md-4 show_report">
                        
                         <button type="button" id="section_submit" class='all_buttons' style="background-color:#C03;float:right; font-weight:bold; height:32px; width:72px;" onClick="window.location.href='<?php echo base_url(); ?>admin/Page_search/<?php if($this->uri->segment(2)=='Page_search'){ echo 'class="selected"';} ?>'" >
					...
					<div class="form_view">
						<h3>Edit Section Of Mobile Search Page</h3>
							<table>						
								<tr>
                                ...
                                <?php
									...
									$img_size=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='mobile' AND page_nm='searchpage' AND culumns_count='$clmn_conunt' ");?>
						   ...
						   $qr_clmnid=$this->db->query("SELECT * FROM mobilesite_columninfo WHERE page_id='5' AND sec_id='$sec_idclmn'  ");
...
function remove_img(i,j,img_sqlid)
{
	...
	if(conf)
	{
				...
				$.ajax({
							method:"POST",
							url:"<?php echo base_url(); ?>admin/Page_search/remove_imagedata",
...
function remove_csvfile(i,j,img_sqlid,cvsfilenm)
{
	...
	if(conf)
	{
		...
		$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/Page_search/remove_csvdata",
