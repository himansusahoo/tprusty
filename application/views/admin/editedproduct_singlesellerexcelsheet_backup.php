...
//										}
	...
	//------------------------------------------------Data Populate of failed product start-------------------------//
							...
							{ 
								...
								if(@$res_editedproduct->shipfee_amount==0)
								{
								...
								$objPHPExcel->getActiveSheet()->SetCellValue('V'.$edited_rowcount, @$res_editedproduct->shipfee_amount);
								...
								$objPHPExcel->getActiveSheet()->SetCellValue('AC'.$edited_rowcount, @$res_editedproduct->country_mafg);
								$objPHPExcel->getActiveSheet()->SetCellValue('AD'.$edited_rowcount, @$res_editedproduct->meta_title);
								$objPHPExcel->getActiveSheet()->SetCellValue('AE'.$edited_rowcount, @$res_editedproduct->meta_keyword);
								$objPHPExcel->getActiveSheet()->SetCellValue('AF'.$edited_rowcount, @$res_editedproduct->meta_descrp);
								
								
						//-------------------------------Product attribute start-----------------------//
							$attrbsku=$res_editedproduct->sku;
							$query_attrb=$this->db->query("SELECT * FROM seller_product_attribute_value WHERE sku='$attrbsku' group by attr_value  ");
							
							if($query_attrb->num_rows()==0)
							
							{$query_attrb=$this->db->query("SELECT * FROM product_attribute_value WHERE sku='$attrbsku' group by attr_value ");}
	...
	$objPHPExcel->getActiveSheet()->getStyle('F3:'.end($attrbhead_exlcoulmnname).$edited_rowcount)->getProtection()->setLocked(PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
...
header("Content-Disposition: attachment; filename=".preg_replace('#"#',"_",preg_replace('#/#',"_",str_replace(',','_',str_replace('&','',str_replace(' ','_',strtolower($catgnm))))))."_".$rand_string."_".$dt_rec."_edited.xls"); 
