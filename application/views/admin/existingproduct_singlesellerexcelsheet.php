...
PHPExcel_Style_Border::BORDER_THIN,'color' => array('argb' => '#A0A0A0'),)));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('K2', 'GST');
...
for ($i = 'A'; $i !=  $objPHPExcel->getActiveSheet()->getHighestColumn(); $i++) {
	...
	$objPHPExcel->setActiveSheetIndex(2);
	$objPHPExcel->getActiveSheet()->setTitle(str_replace("'", "",stripslashes(preg_replace('#"#',"_",preg_replace('#/#',"_",str_replace(' ','_',strtolower(substr($catgnm,0,29))))))));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('I1', 'YYYY-MM-DD Or YYYY/MM/DD');	
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('L2', 'GST');	
...
header("Content-Disposition: attachment; filename=".str_replace("'", "",stripslashes(preg_replace('#"#',"_",preg_replace('#/#',"_",str_replace(',','_',str_replace('&','',str_replace(' ','_',strtolower($catgnm))))))))."_".$rand_string."_".$dt_rec."_existingproduct.xls");  
