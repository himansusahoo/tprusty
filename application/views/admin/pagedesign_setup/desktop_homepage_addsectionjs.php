...
function add_columninfo()
{
	...
	else
	{
		//-------------------------Disable section Data start-----------------//
			...
			//var sec_bgclr=$('#sectionbackg_clr').val();
			//alert(sec_bgclr);
			//$('#lbl_sectionbackg_clr').css('background-color',''+sec_bgclr+'' );
			//$('#sectionbackg_clr').css('display', 'none');
			//$("#sectionbackg_clr").attr('readonly','readonly');
			
			$('#lbl_col_num').text($('#col_num').val());
			$('#col_num').css('display', 'none');		
			
			$('#lbl_slctimage_size').text($('#slctimage_size option:selected').text()+' Pixel');
        ...
        fileref.setAttribute("src", '<?php //echo base_url().'jquery_date_picker/jquery-ui.js';?>');		
		document.getElementsByTagName("head")[0].appendChild(fileref);
		
		var fileref=document.createElement('script');
        fileref.setAttribute("type","text/javascript");
        fileref.setAttribute("src", '//code.jquery.com/jquery-1.10.2.js');		
		document.getElementsByTagName("head")[0].appendChild(fileref);
        ...
        fileref.setAttribute("href", '<?php //echo base_url().'jquery_date_picker/jquery-ui.css';?>')
		document.getElementsByTagName("head")[0].appendChild(fileref);
		
		var fileref=document.createElement("link")
        fileref.setAttribute("rel", "stylesheet")
        fileref.setAttribute("type", "text/css")
        fileref.setAttribute("href", '/resources/demos/style.css')
		document.getElementsByTagName("head")[0].appendChild(fileref);
...
{ 
	...
	//var flnm = $('#imgInp'+i+j+'').prop(input)['name'];
	//alert(flnm);
	//var re = new RegExp("^[^<>%$#%@|]*$\'\"\(\)");
	//------check image or not start-----------------//
	...
	/*var filename = $('#imgInp'+i+j+'').val();
	if(!checkSpecialCharacter(filename))
	{
		
		$('#alrt'+i).text('! Special Characters are not Allow in image file name , Please rename it and try to upload again ');
		$('#alrt'+i).css('display','block');
		 $('html, body').animate({
        'scrollTop' : $('#fld_secion_'+i).position().top
   		});	
...
function valid_img_slider_banner(width,height,i,j) {
	...
	var validate_img_size=$('#slctimage_size option:selected').text();
...
function valid_imgdimensiion(width,height,i,j) {
	...
	var imgdmns_defnd=$('#slctimage_size option:selected').text();	
	//alert(imgdmns_defnd);
	var image_dm=width+'x'+height;
...
function populate_sectiondatatype(sec_type)
{
	if(sec_type=='Rich Text Editor')
	{
		...
		$('#div_screenshot').append($("<img></img>").attr("src","<?php echo base_url() ?>/images/admin/desktop/desktop_homepage/Rich_texteditor.png"));
		$('#screeshotprocess_div').css('display','none');
		return false;
	}
	
	
	if(sec_type=='New Arrivals' || sec_type=='Trending Products' || sec_type=='Recently Viewed Items')
	{
		$("#sectiondata_type").html('');
		...
		$('#div_screenshot').append($("<img></img>").attr("src","<?php echo base_url() ?>/images/admin/desktop/desktop_homepage/proudct.png"));
		$('#screeshotprocess_div').css('display','none');
	}
	else
	{
			...
			if(sec_type=='Banner' || sec_type=='Slider' || sec_type=='Video' || sec_type=='Featured Box')
			{
...
function select_nofcolumn()
{
	...
	if($('#section_type').val()=='Slider' && $('#sectiondata_type').val()=='Banner')
	{
		...
		$('#col_num').append($("<option></option>").attr("value","4").text("4"));
		$('#col_num').append($("<option></option>").attr("value","5").text("5"));
...
function select_imagesize()
{
	...
	else
	{
		...
		$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/desktop_page_setup/select_imagesize",
...
function populate_screenshot(img_size)
{
	...
	$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/desktop_page_setup/select_screeshot",
...
{
	if($('#section_type').val()!='Video' && $('#section_type').val()!='Product' && $('#section_type').val()!='Prodcts Vertical section'  &&
	$('#section_type').val()!='New Arrivals' && $('#section_type').val()!='Trending Products' && $('#section_type').val()!='Recently Viewed Items' && $('#section_type').val()!='Rich Text Editor' && $('#section_type').val()!='Video with Product Carousel' && $('#section_type').val()!='Product Grid View')
	{
