...
{ 
	...
	if($.inArray(ext, ['png','jpg','jpeg']) == -1) 
	{
   	...
   	$('#alrt'+i).text('! Invalid Image File. Please Upload Image file with extension .jpg / .jpeg / .png ');
		$('#alrt'+i).css('display','block');
		 $('html, body').animate({
        'scrollTop' : $('#fld_secion_'+i).position().top
    });	
...
function populate_sectiondatatype(sec_type)
{
	if(sec_type=='Rich Text Editor')
	{
		...
		$('#div_screenshot').append($("<img></img>").attr("src","<?php echo base_url() ?>/images/admin/mobile/mobile_homepage/Rich_texteditor.png"));
	...
	if(sec_type=='New Arrivals' || sec_type=='Trending Products' || sec_type=='Recently Viewed Items')
	{
		...
		$('#div_screenshot').append($("<img></img>").attr("src","<?php echo base_url() ?>/images/admin/mobile/mobile_homepage/product_vertical_section.png"));
		$('#screeshotprocess_div').css('display','none');
	}
	else
	{
...
function select_nofcolumn()
{
	...
	/*if($('#section_type').val()=='Carousel' && $('#sectiondata_type').val()=='Product')
	{
		$("#col_num option[value='1']").remove();
		$("#col_num option[text='1']").remove();
		$("#col_num option[value='2']").remove();
...
function select_imagesize()
{
	...
	else
	{
		...
		$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/page_setup/select_imagesize",
...
function populate_screenshot(img_size)
{
	...
	$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/page_setup/select_screeshot",
...
function valid_columndata()
{
	...
	$('#section_type').val()!='New Arrivals' && $('#section_type').val()!='Trending Products' && $('#section_type').val()!='Recently Viewed Items' && $('#section_type').val()!='Rich Text Editor')
	{
