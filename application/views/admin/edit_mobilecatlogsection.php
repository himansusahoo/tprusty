...
function remove_menulink(pgsection_sqlid,menu_id)
{
	...
	if(conf){
		$('#loader_div').css('display','block');
			$.ajax({
				method:"POST",
				url:"<?php echo base_url().'admin/Page_catlog/remove_categorymenulink' ?>",
			...
			<div id="content">  
                ...
                <?php require_once('pagedesign_setup/mobile_catlog_addsectionjs.php'); ?>
                	<?php
					//$attributes = array('onSubmit' => 'return valid_columndata()');
					echo form_open_multipart('admin/Page_catlog/update_catlogectiondata');
					?>
						...
						<h3>Edit Section Of Catalog Page</h3>
										   ...
										   ?>
                                            <tr>
                                                ...
                                                <?php
													...
													$qr_parent=$this->db->query("SELECT parent_id,label_name FROM category_menu_mobile WHERE dskmenu_lbl_id='$parent_id'");
                            ...
                            <tr>
                             ...
                             <?php } ?>   
                                <tr>
                                    ...
                                    <label id="lbl_section_type" style="font-size:18px;"><?=$sec_info->row()->sec_type?> </label>
                                    	<select name="section_type" id="section_type" class="text2" style="display:none">
                                            ...
                                            <option value="Product Catalog" <?php if($sec_info->row()->sec_type=='Product Catalog'){echo 'selected';} ?>>Product Catalog</option>
                                            <option value="Products Vertical section" <?php if($sec_info->row()->sec_type=='Products Vertical section'){echo 'selected';} ?>>Products Vertical section</option>
                                ...
                                <?php
									...
									$img_size=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='mobile' AND page_nm='catalog' AND culumns_count='$clmn_conunt' ");?>
                            ...
                            <?php } ?>                
                                ...
                                <tr>
                                   ...
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/mobile/mobile_catlog/product_product_clmn2_300x400.png'; ?>" >								
                                   <?php } ?>
						   ...
						   $qr_clmnid=$this->db->query("SELECT * FROM mobilesite_columninfo WHERE page_id='3' AND sec_id='$sec_idclmn'  ");
						   ...
						   if($qr_clmnid->num_rows()>0){
                                ...
                                <tr>
                                <td style="width:20%;"> Memo </td>
                                <td><textarea class="text2" name="clmn_memo[]" id="clmn_memo<?=$i?>"><?php $res_clmninfo['memo']?></textarea></td>
                                </tr>
                                <tr>
								...
								$clmn_sqlid=$res_clmninfo['clmn_sqlid'];								
								 ?>
                                 <?php 
								 	if($sec_info->row()->sec_type!=='Video' && $sec_info->row()->sec_type!=='Product' && $sec_info->row()->sec_type!=='Prodcts Vertical section'&& $sec_info->row()->sec_type!=='Product Catalog')
									{
								  ?>
...
function remove_img(i,j,img_sqlid)
{
	...
	$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_catlog/remove_imagedata",
...
function remove_csvfile(i,j,img_sqlid,cvsfilenm)
{
	...
	if(conf)
	{
		...
		$.ajax({
			method:"POST",
			url:"<?php echo base_url(); ?>admin/Page_catlog/remove_csvdata",
