			 ...
			 <!-- @end top-bar  -->
                ...
                <?php /*?><?php if($this->session->flashdata('flshmsg')){ ?>	<div id="successfully_verify" style="color:#0C0;" align="center">
					...
					<div class="row gray_bg">						
						...
						<img src="<?php //echo base_url().'images/success_icon.png' ?>">&nbsp<?php //echo $this->session->flashdata('msgcod_wtchrg'); ?></div> <?php //} ?>									<?php /*?><button id="product_submit" class='seller_buttons' style="background-color:#666;float:right;" onclick="window.location.href='<?php echo base_url().'admin/Desktop_page_single_product/addnewsection_desktop_signle_product'; ?>'" >
							<i class="fa fa-plus-square" aria-hidden="true" style="color:#FFF;"></i> &nbsp;Add New Section 
           				</button> <?php */?>  
                        
                        <a id="product_submit" class='seller_buttons add-new-sec' style="background-color:#666;float:right;" href="<?php echo base_url().'admin/Desktop_page_single_product/addnewsection_desktop_signle_product'; ?>" target="_blank">
                        ...
                        <div>
                           <span>
                           		 <?php 
								 $qr_section=$this->db->query("SELECT menu_id FROM desktopsite_pagesectioninfo WHERE page_id='4' AND page_name='single product' AND menu_id!='' ");
...
{ var mnulinkid=$('#menu_link').val();
	...
	else
	{
		$('#loader_div').css('display','block');
					$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/populate_singleprodpagedesigninfo",
...
function singleprodsortby_section_upajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/single_productsortby_section_up",
...
function singleprodsortby_section_downajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/single_productsortby_section_down",
					data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
					success: function (data) {						
...
function singleprodsortby_section_totopajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/single_productsortby_section_totop",
					data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
					success: function (data) {						
...
function singleprodsortby_section_todownajx(pgsec_sqlid)
{
	var menu_id=$('#menu_link').val();
			...
			$('#process_div'+pgsec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/single_productsortby_section_todown",
					data:{pgsec_sqlid:pgsec_sqlid,mnulinkid:menu_id},
					success: function (data) {						
	...
	if(conf)
	{
		var menu_id=$('#menu_link').val();
			...
			$('#process_div'+sec_sqlid).css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/remove_desktop_singleproduct_section",
	...
	if($('#sec_status').val()=="")
		...
		else
		{
			...
			$('#loader_div').css('display','block');
				$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Desktop_page_single_product/change_sec_status",
