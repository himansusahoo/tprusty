  ...
  function apply_delete(){
	var selected_catg_id = $('input[name="chk_sku[]"]:checked').map(function(_, el){
		...
		if(ys){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>admin/Cache/dltselectedcache_folder_update",
				...
				success: function (data) {
					...
					//if(data == 'success'){
						...
						window.location.href='<?php echo base_url(); ?>admin/Cache/dltcachefld';
...
{
	var conf=confirm('DO You Want To Delete Home Page Cache');
	if(conf)
	{
		$('.allprocess_div_catg').css('display','block');
		$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/Cache/dlthomepagecache",
						data:{dlt_all:"dlt_all"},
						beforeSend: function(){
							$('#cachetable').hide();
							$('#lodr_imgcache').show();  
...
function dltcache_folder(catg_id)
{
	...
	if(conf)
	{
		...
		$.ajax({
						method:"POST",
						url:"<?php echo base_url(); ?>admin/Cache/dltcache_folder_update",
...
$(document).ready(function(){
		...
		$('input:checkbox').prop('checked', this.checked);
			...
			<div id="content">  
				...
				<div class="main-content">
                    <div id="keywordlist">
                    <div style="font-size:20px; margin-top:10px;">Manage Cache <button title="Flush All Cache" onClick="dltallcache_folder()" class="all_buttons">Delete All Cache </button><button title="Delete Home Page Cache" onClick="dlthomepagecache()" style="background-color: #ae06e5;" class="all_buttons">Delete Home Page Cache </button></div>
    ...
    margin-top: 68px;" onClick="apply_delete()" class="seller_buttons" id="indxsearch_btn"  ><i class="fa fa-times" aria-hidden="true"></i>&nbsp;Delete</button>
              ...
              <img src="<?php echo base_url(); ?>images/loader.gif" id="lodr_imgcache" class="list_view_lodr_img" style="display:none; margin-left:525px;">
              </div>
              <div id="cachetable" style="display:block;">
                     <table width="500" class="table table-bordered table-hover" > 
                            ...
                            <tr class="table_th " style="background-color:#099">								
                             ...
                             <?php $attrb=array('method'=>'GET'); echo form_open('admin/Cache/show_search_cache',$attrb); ?> 
							 <tr class="filter_tr">
								<td><input type="checkbox" id="check_all"/> </td>
								<td></td>
								<td>
                                <select class="chosen-select" name="lvlid[]" multiple id="" required>
										
                                        <?php foreach($cache_listforsearch->result_array() as $res_cache_list){ ?>
										<option value="<?=$res_cache_list['catg_id']?>">
										<?php  $catid=$res_cache_list['catg_id'];
                               ...
                               <button type="button" onClick="window.location.href='<?php echo base_url().'admin/Cache/dltcachefld' ?>'" 
