...
function remove_menulink(pgsection_sqlid,menu_id)
{
	...
	if(conf){
		$('#loader_div').css('display','block');
			$.ajax({
				method:"POST",
				url:"<?php echo base_url().'admin/Page_category/remove_categorymenulink' ?>",
			...
			<div id="content">  
                ...
                <?php require_once('pagedesign_setup/mobile_category_addsectionjs.php'); ?>
                	<?php
					//$attributes = array('onSubmit' => 'return valid_columndata()');
					echo form_open_multipart('admin/Page_category/update_categoryectiondata');
					?>
                	<!--<form onSubmit="return membershipInfo()">-->
					<div class="row content-header">
						<div class="col-md-8"><h4><b>Edit Section Of Categories Page</b></h4><div id="ssmessg"><?= $this->session->flashdata('succss_msg'); ?></div></div>
						<div class="col-md-4 show_report">
                         <button type="button" id="section_submit" class='all_buttons' style="background-color:#C03;float:right; font-weight:bold; height:32px; width:72px;" onClick="window.location.href='<?php echo base_url(); ?>admin/Page_category/<?php if($this->uri->segment(2)=='Page_category'){ echo 'class="selected"';} ?>'" >
					...
					<div class="form_view">
						<h3>Edit Section Of Categories Page</h3>
							<table>
                            <tr>                            
                            <td colspan="2">
                            	<!------------------------ Menu link table start----------------------------->
								<?php if($sec_info->row()->menu_id!='') {?>
										   ...
										   ?>
                                            <tr>
                                                ...
                                                <?php
													...
													$qr_child=$this->db->query("SELECT label_name,parent_id FROM category_menu_mobile WHERE dskmenu_lbl_id='$vsal'");
													
													$child_menulabel=$qr_child->row()->label_name;
													$parent_id=$qr_child->row()->parent_id;
													
													$qr_parent=$this->db->query("SELECT label_name FROM category_menu_mobile WHERE dskmenu_lbl_id='$parent_id'");
													$parent_menulabelname=$qr_parent->row()->label_name;
													
													echo $parent_menulabelname.' >> '.$child_menulabel;
												?>
                                               
                                                </td>
                                                <td><i class="fa fa-times" onClick="remove_menulink('<?=$pgsection?>','<?=$vsal?>')" title="Remove Menu Link" style="cursor:pointer; color:#F00;"></i></td>
                                            </tr>
                            	...
                            	<tr>
                                    <td> Second Level Menu Link</td>
                                    <td>
                                        <select name="menu_link[]" data-placeholder="Choose Category Links"  class="chosen-select" style="width:490px" multiple tabindex="4" >
											<?php
                                                 if($sel_catg->num_rows()>0)
                                                    foreach($sel_catg->result_array() as $rw){																			
                                                    $dskmenu_lbl_id=$rw['parent_id'];
                                                    $qr_parent=$this->db->query("SELECT label_name FROM 
                                                    category_menu_mobile WHERE dskmenu_lbl_id='$dskmenu_lbl_id' ");
                                                    $parent_lblname=$qr_parent->row()->label_name;
                                                         ?>
													 <?php
                                                        if(!in_array($rw['dskmenu_lbl_id'],$ext_menuid)){
															if($parent_lblname=='Life Style')
															{
																$thirdlvl_parentid=$rw['dskmenu_lbl_id'];
																$qrfourth_child=$this->db->query("SELECT dskmenu_lbl_id,parent_id,label_name FROM 
																category_menu_mobile WHERE parent_id='$thirdlvl_parentid' ");
																$thirdlvlchild_lblname=$qrfourth_child->row()->label_name;
																$thirdlvl_dsklblid=$qrfourth_child->row()->dskmenu_lbl_id;
                                                      ?>
													<option value="<?=$thirdlvl_dsklblid;?>"><?=$parent_lblname." >> ".$rw['label_name']." >> ".$thirdlvlchild_lblname;?></option>
														<?php } else{ ?>                                                      
                                                    <option value="<?=$rw['dskmenu_lbl_id'];?>"><?=$parent_lblname." >> ".$rw['label_name'];?></option>
													<?php } } ?>
                                            <?php }?>
                                        </select>
                                    </td>
								...
								<tr>
                                    ...
                                    <label id="lbl_section_type" style="font-size:18px;"><?=$sec_info->row()->sec_type?> </label>
                                    	<select name="section_type" id="section_type" class="text2" style="display:none">
                                            ...
                                            <option value="Featured Box" <?php if($sec_info->row()->sec_type=='Featured Box'){echo 'selected';} ?>>Featured Box</option>
                                            <!--<option value="Prodcts Vertical section" <?php //if($sec_info->row()->sec_type=='Prodcts Vertical section'){echo 'selected';} ?>>Prodcts Vertical section</option>-->
                                            <option value="Category" <?php if($sec_info->row()->sec_type=='Category'){echo 'selected';} ?>>Category</option>
                                            <!--<option value="Popular Product" <?php //if($sec_info->row()->sec_type=='Popular_Product'){echo 'selected';} ?>>Popular Product</option>-->
                                            <!--<option value="Category Product" <?php //if($sec_info->row()->sec_type=='Category_Product'){echo 'selected';} ?>>Category Product</option>-->
                                ...
                                <?php
									...
									$img_size=$this->db->query("SELECT * FROM pagedesign_imagesize WHERE display_type='mobile' AND page_nm='category' AND culumns_count='$clmn_conunt' ");?>
                            ...
                            <?php } ?>                
                                ...
                                <tr>
                             	...
                             	<div id="div_screenshot" align="center">
                             			 <?php
								   if($sec_info->row()->sec_type!='Catagory' && $sec_info->row()->sec_type!='Popular Product' && $sec_info->row()->sec_type!='Category Product'  ){	     
								   $sec_type=str_replace(' ','_',$sec_info->row()->sec_type);
								   $col_num=$sec_info->row()->nos_column;
								   $img_size=$sec_info->row()->image_size;
								   $qr=$this->db->query("SELECT ".$sec_type." as srcshot_imgname FROM pagedesign_imagesize WHERE display_type='mobile' AND page_nm='category' AND culumns_count='$col_num' AND img_size='$img_size' ");								  
								   ?>
                                   <?php if($sec_info->row()->sec_type=='Prodcts Vertical section') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/mobile/mobile_category/product_vertical_section.png'; ?>" >									
                                   <?php } ?>
                                    <?php if($sec_info->row()->sec_type=='Video') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/mobile/mobile_category/video.png'; ?>" >								
                                   <?php } ?>
                                   
                                    <?php if($sec_info->row()->sec_type=='Product') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/mobile/mobile_category/product_product_clmn2_300x400.png'; ?>" >								
                                   <?php } ?>
                                   
                                   <?php if($sec_info->row()->sec_type=='Rich Text Editor') {?>
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/mobile/mobile_category/Rich_texteditor.png'; ?>" >								
                                   <?php } ?>
                                   ...
                                   <img style="float:left;" src="<?php echo base_url().'images/admin/mobile/mobile_category/'.$qr->row()->srcshot_imgname; ?>" >									<?php } ?>
						   ...
						   $qr_clmnid=$this->db->query("SELECT * FROM mobilesite_columninfo WHERE page_id='2' AND sec_id='$sec_idclmn'  ");
						   ...
						   if($qr_clmnid->num_rows()>0){
                                ...
                                <?php /*?><i style="color:#FF0000;cursor:pointer;" class="fa fa-times" aria-hidden="true"  onClick="remove_culmn('<?=$i?>')"></i><?php */?></legend>
                                ...
                                <select style="width:150px; height:40px;" class="text2" name="sku_sortby<?=$i?>[]" id="sku_sortby<?=$i.$j?>">
                                    ...
                                    <option value="pric_dec" <?php if($res_imgdata['sort_by_info']=='pric_dec'){echo "selected";} ?>>Price Low To High</option>                                </select></fieldset>
                                </td>
                                <td>
...
function remove_img(i,j,img_sqlid)
{
	...
	$.ajax({
					method:"POST",
					url:"<?php echo base_url(); ?>admin/Page_category/remove_imagedata",
...
function remove_csvfile(i,j,img_sqlid,cvsfilenm)
{
	...
	if(conf)
	{
		...
		$.ajax({
				method:"POST",
				url:"<?php echo base_url(); ?>admin/Page_category/remove_csvdata",
