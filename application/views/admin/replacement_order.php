...
function transfer_ordervalid()
{
		...
		if(count==0)
		{
		 ...
		 var conf=confirm('Do you want to replace order ?');
		 if(conf==false)
		 {
			return false;	 
		}
	...
	if(conf)
	{
		window.location.href='<?php echo base_url().'admin/sales/cancel_transfered_order/' ?>' + old_ordrid;	
			...
			<div id="content">    
                ...
                <div id="tab4" >
                	<h3>Order History: <?php echo $replace_orderid; ?></h3>
					<div id="ss"></div>
                    <table class="table table-bordered table-hover">
                                ...
                                <?php 
								$total_price=0;
								 $qr5=$this->db->query("select * from ordered_product_from_addtocart where order_id='$replace_orderid' group by sku"); 
					...
					//    print_r($rw_sellername->business_name);}
					   echo "Seller Name: ". $rw_sellername->business_name; }
									...
									<td>  <?php $qr9=$this->db->query("select * from ordered_product_from_addtocart where product_id='$rw5->product_id' and                                                 sku='$rw5->sku' and order_id='$replace_orderid'  ");  
								  //$row3=$qr3->row();
								  $price=0;
								  $rw9=$qr9->row(); 
								...
								$qr11=$this->db->query("select * from order_info where order_id='$replace_orderid' ");  
									  $rec11=$qr11->result();
               ...
               <div class="row content-header">
						...
						<div class="col-md-8"><h3>Order Replacement</h3></div>
						<div class="col-md-4 show_report">
							<!--<button type="button"  class="all_buttons">Create New Order</button>-->
						</div>
                        
					</div>
                    <form action="<?php echo base_url().'admin/sales/reassign_order_replacement' ?>" method="POST" onSubmit="return transfer_ordervalid()"   >
               <!-- Fixed Total Ordered Amount <input type="text" name="ordered_mount" id="ordered_mount">-->
                
                 <input type="hidden" name="old_order_id" value='<?php echo $replace_orderid ?>' >                 
                <input type="submit" name="btn_transferorder" value="Reassign Order"    >
						...
						if($replace_productdata){
							$i=0;
						foreach($replace_productdata as $resReplacedata)
						{
								
						?>
                                      
                  <tr>
                  <td><input type="checkbox" name="chek_sku[]" id="chek_sku<?= $i ?>" value="<?= $resReplacedata->sku; ?>"  onClick="chk_record(<?= $i ?>)">
                 <span style="display:none;">
                  <input type="checkbox" name="product_id[]" id="product_id<?= $i ?>" value="<?= $resReplacedata->product_id; ?>"> 
                  <input type="checkbox" name="user_id[]" id="user_id<?= $i ?>" value="<?= $resReplacedata->user_id; ?>"> 
                   <input type="checkbox" name="buyer_qnt[]" id="buyer_qnt<?= $i ?>" value="<?= $resReplacedata->cus_qantity; ?>"> 
                  
                  </span>
                  
                  </td>
                  
                     <td><?php $image=explode(',',$resReplacedata->imag); ?><img src="<?php echo base_url().'images/product_img/'.$image[0]; ?>" width="30" class="list_img">
                     <?= $resReplacedata->name; ?>
                     </td>
                       <td>Rs.<?= $resReplacedata->mrp; ?></td>
                       <td>Rs.<?= $resReplacedata->price; ?></td>
                       <td>Rs.<?= $resReplacedata->special_price; ?></td>
                           
                       <td>Rs.<?= $resReplacedata->shipping_fee_amount; ?></td>
                       <td><?= $resReplacedata->cus_qantity; ?></td> 
                        <td><?= $resReplacedata->quantity; ?></td> 
                       <td><?= $resReplacedata->stock_availability; ?></td> 
                       
                       <td><?= $resReplacedata->approve_status; ?></td>
                        <td><?= $resReplacedata->business_name  ?> </td>
