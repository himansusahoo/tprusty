	...
	$.ajax({
       ...
       <div class="info-products">
		...
		?>
				  <div class="col-md-4 pro-grid">
				     <div class="box-in">
							<div class="grid_box">	
                             ...
                             <?php }else { ?>
                             <img src="<?php echo base_url();?>images/product_img/<?=$image_arr[0];?>" onerror="imgError(this);"  alt="" class="wow fadeInDown">                       
                             <?php } ?>
                             </div>	
                            ...
                            <div class="outofstock">
                             <a href="<?php echo base_url().'product_description/product_detail/'.preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" ><span> Out Of Stock </span></a>
                            </div>
                            <?php }?>
					  </div>
				  </div>
				...
				<input type="button" class="btn-sign-in"  value="View more" name="button" onClick="ShowMoreData('<?=$no_of_product;?>','<?=$this->uri->segment(4);?>','<?php if($this->uri->segment(5) != ''){ echo $this->uri->segment(5);}else{ echo 'NOT';}?>')">
				<?php }	?>
           </div>
