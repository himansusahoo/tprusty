...
function show_paytmonlinepayment(){
	$.ajax({
		url:'<?php echo base_url();?>mycart/load_paytm',
		method:'post',
		data:{adj_type:'O'},
		success:function(result){
			//$('#ajxtst').html(result);
			...
			$('#onlinepaytm_div').html(result);
			$('#onlinepaytm_div').css('display','block');
			$('#online_paytm_link').html('<a  onClick="alert(\'You can use online payment option once in an order.\');" style="cursor:pointer;"> <i class="fa fa-credit-card"></i> PAY BY PAYTM </a>');
		}
	});
}
...
<td colspan="2" align="center"><a class='inline2' href="#inline_content_add_address"><button class="button btn-cart-big" type="button"> <i class="fa fa-plus-square"></i>
...
<div class="address_dv">
	...
	<div class="multi_address_dv_inn">
        ...
        <span class="filter-form" >
           <label class="radio">
            <input type="radio" onclick="setDefaultaddress(<?=$row_addrss->address_id; ?>)" <?php if($cus_data!=""){if($address_id == $row_addrss->address_id){ echo 'checked';} } ?> name="addrs"> <i></i> Default Address </label> 
                ...
                <td>
                    <select name ="state" id ="state<?=$sl; ?>" class="input-text">
                        <option value="">---select---</option>
                         ...
                         <option value="<?=$state->state_id; ?>" <?php if($state->state_id == $row_addrss->state_id){ echo 'selected';} ?>><?=$state->state; ?></option>
                         <?php } ?>
                    </select>
                </td>
            </tr>
...
<div class="line">&nbsp;</div>
  ...
  <tr>
   ...
   $image_cart=explode(',',$rw1->imag);
    ...
    ?> <a href="<?php echo base_url().preg_replace('#/#',"-",str_replace(" ","-",strtolower($rw1->name))).'/'.$rw1->product_id.'/'.$rw1->sku  ?>" target="_blank"><img src="<?php echo base_url().'images/product_img/'.'catalog_'.$image_cart[0]; ?>"  width="30" alt="<?=$rw1->name;?>"></a></div>
    <div class="chckout-desc"> 
    <?php  $qr2=$this->db->query("select name,weight from product_general_info where product_id='$rec_cart->product_id' GROUP BY product_id ");
   $rw2=$qr2->row();
  // product weight for COD charge 
...
//    print_r($rw_sellername->business_name);}
   ...
   ?>
    ...
    <h4> Rs. <?php  $user_id=$this->session->userdata['session_data']['user_id'];
	  ...
	  foreach($qr3->result() as $rw_price)
	  {
		...
		$singleprod_price_arr[]=$price+$rec4[0]->shipping_fee_amount;	
	  }
  ...
  </tr><?php $total_price=ceil($total_price+$subtotal_price); 
...
<h4> Payment Mode </h4>
<ul>
<li><a id="wallet_link" onClick="pay_by_wallet(); " style="cursor:pointer;" > <span class="wallet"> &nbsp;</span> PAY BY WALLET </a></li>
...
<li><a id="cod_link" onClick="pay_by_cod('<?=$cus_data->pin_code?>'); DrawCaptcha();" style="cursor:pointer;" > <i class="fa fa-inr"></i> COD </a></li>
</ul>
</div>
...
<tr>
   ...
   <?php  echo " Rs.".round($total_price);
	...
	?>
		...
		$cod_totalprice_arr=array();
		$cod_totalpricewithtax_arr=array();
		$total_weightcharge_arr=array();
		$total_taxchrgetobuyer_arr=array();
		$total_chargetocustomer_arr=array();
		$total_chargetomoonboy_arr=array();
		$totatl_discounttobuyer_arr=array();
		...
		for($codj=0;$codj<count($prod_weight); $codj++)
		{
			...
			if($wtchrg_query->num_rows()>0)
			{
				...
				$total_weightcharge_arr[]=$cod_chargeaswtgheach;
				//-------------------amount to be charge start--------------------------
						...
						if($cod_percen_query->num_rows()>0)
						{
							...
							$cod_percen_charge=$cod_percen_row->Percentage_charge;
							 ...
							 $cod_totalprice_arr[]=$singleprod_price_arr[$codj]+round(($cod_chargeaswtgheach/100)*$cod_percen_charge);
							 
							 $total_weightcharge=$cod_chargetobuyer;
							  // total weight for session
							 
							 $total_chargetocustomer=$cod_chargetobuyer;
							 $total_chargetocustomer_arr[]=round(($cod_chargeaswtgheach/100)*$cod_percen_charge); // total charge to buyer for session
							 
							 if($cod_percen_charge!='100')
							 {$total_chargetomoonboy=$total_chargetomoonboy+round(($cod_chargeaswtgheach/100)*(100-$cod_percen_charge));
							 	$total_chargetomoonboy_arr[]=round(($cod_chargeaswtgheach/100)*(100-$cod_percen_charge));
							 }else
							 {$total_chargetomoonboy_arr[]='0';}
						...
						{$cod_totalprice=$cod_totalprice+$cod_chargeaswtgheach; $cod_totalprice_arr[]=$singleprod_price_arr[$codj]+$cod_chargeaswtgheach;}
			...
			foreach($cod_totalprice_arr as $ky=>$val)
			{	$cod_totalpricewithtax_arr[]=$val+$total_taxchrgetobuyer;
				$total_taxchrgetobuyer_arr[]=$total_taxchrgetobuyer;
				$totatl_discounttobuyer_arr[]=$totatl_discounttobuyer;
			}
		//------------------------------------COD charges end-----------------------------------------
   ...
   <?php 
	...
	$moonboy_trans_id=random_string('numeric',2).$dt;
		...
		$this->session->set_userdata('sesscodtotal_price_arr', implode('-',$cod_totalpricewithtax_arr));  
		$this->session->set_userdata('sesstotal_weightcharge_arr',implode('-',$total_weightcharge_arr));   
    	$this->session->set_userdata('sesstotal_taxchrgetobuyer_arr', implode('-',$total_taxchrgetobuyer_arr));   
    	$this->session->set_userdata('sesstotal_chargetocustomer_arr',implode('-',$total_chargetocustomer_arr));		
		$this->session->set_userdata('total_chargetomoonboy_arr',implode('-',$total_chargetomoonboy_arr));		
		$this->session->set_userdata('sesstotatl_discounttobuyer_arr',implode('-',$totatl_discounttobuyer_arr));
...
<div id="onlinepaytm_div" style="display:none;" align="center"></div>
...
<div id="payment_div_wallet">
    ...
    <table width="100%">
    	<tr>
        	<td width="55%">
            	<span class="wlt-amnt"> Total Wallet Balance: <?php if($query_wallet->num_rows()!=0){ echo "<strong class='tw'>Rs.".$curr_wallet_balance.'</strong>'; }else {echo "<strong>Rs.0</strong>";} ?> </span><hr/>
            ...
            <td width="45%" class="rht_td">
            	...
            	<?php }else{?>
            	<span class="wlt-amnt"> Available Wallet Balance: <?php if($query_wallet->num_rows()!=0){ echo "<strong id='avail_wlt_bal'>Rs.".$curr_wallet_balance.'</strong>'; }else {echo "<strong>Rs.0</strong>";} ?> </span><hr/>
