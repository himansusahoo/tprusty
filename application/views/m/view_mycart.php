				  ...
				  <div class="checkout" id="checkout">
					...
					<ul class="icon1 sub-icon1 profile_img">
						...
						if($qr1->num_rows()==0)
						{
							...
							$qr1=$this->db->query("select a.product_id,a.name,b.sku,b.quantity,b.status,b.approve_status,c.imag from product_general_info a inner join product_master b on a.product_id=b.product_id inner join product_image c on a.product_id=c.product_id where a.product_id='$rec_cart->product_id'");
						}
								  ...
								  $user_id=$this->session->userdata['session_data']['user_id'];
                                   ...
                                   $rec_ct=$qr2->num_rows(); 
                                    ...
                                    <div class="quantity" style="width:80%;">
									 ...
									 <script>
										$(function () {
											$('.add<?=$sl;?>').on('click',function(){
												var $qty=$(this).closest('p').find('.quantity_added<?=$sl;?>');
												var currentVal = parseInt($qty.val());
												if (!isNaN(currentVal)) {
													$qty.val(currentVal + 1);
												}
											});
											$('.minus<?=$sl;?>').on('click',function(){
												var $qty=$(this).closest('p').find('.quantity_added<?=$sl;?>');
												var currentVal = parseInt($qty.val());
									...
									<p>
										<img style="float:left;margin-top: 8px;" src="http://i.imgur.com/yOadS1c.png" id="minus1" width="20" height="20" class="minus<?=$sl;?>"/>
                                        <input type="text" step="1" min="1" class="quantity_added<?=$sl;?>" id="quantity_added<?=$sl;?>" name="quantity_added" value="<?php echo ($rec_ct); ?>" placeholder="qty" style="float: left; width: 43%;margin-left: 10px; border: 1px solid #ccc; padding: 3px; margin-right: 5px;" />
                                        <img style="float:left;margin-top: 8px;"  id="add1" src="http://i.imgur.com/98cvZnj.png" width="20" height="20" class="add<?=$sl;?>"/>
                                        <input type="button"  value="save" class="btn-success add-btn" style="float:right; width: 22%;" onClick="valid_function('<?=$product_id1;?>','<?=$sku_id1;?>','<?=$session_id;?>','<?=$user_id;?>','<?=$rec_ct;?>','<?=$rec_cart->addtocart_id;?>','<?=@$quantity_table;?>','<?=$sl;?>')" id="submit1<?=$sl;?>">
									</p>
                                    ...
                                    <?php if($avail_qnt<=0  || $avail_status=='Disabled' || $avail_approve_status=='Inactive' || $seller_status!='Active') { if($avail_qnt<=0){?> <span style="background-color:#900; color:#FFF; font-weight:bold;">Out Of Stock. </span> <?php }else{ ?>
    <span style="background-color:#900; color:#FFF; font-weight:bold;">Product is currently Discontinued. </span>
    <?php }} ?>
									    <div class="delivery">
									...
									?>
                                         ...
                                         <?php
											...
											if($avail_qnt>0 && $avail_status=='Enabled' && $avail_approve_status=='Active' && $seller_status=='Active') {  $total_price=ceil($total_price+$subtotal_price);}
