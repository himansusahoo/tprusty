...
function ShowMoreData(result_no,cat_id,lastseg){
	...
	$.ajax({
		url:'<?php echo base_url(); ?>demo/show_more_catalog_data',
		method:'get',
	...
	if(lastseg == 'NOT'){
       ...
       <div class="info-products">
		...
		?>
                ...
                <?php $catgstr_urlpass='offer'; if($no_of_product > $sl){ ?>
				<input type="button" class="btn-sign-in"  value="View more" name="button" onClick="ShowMoreData('<?=$no_of_product;?>','<?=$catgstr_urlpass?>','<?php if($this->uri->segment(2) != ''){ echo $this->uri->segment(2);}else{ echo 'NOT';}?>')">
				<?php }	?>
           </div>
               <br>
