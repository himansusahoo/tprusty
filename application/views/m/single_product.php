...
.flexslider .slides img {
    ...
    <script>
		...
		function addtoWishlist_temp(product_id,sku){
			$.ajax({
...
<div class="body-back">
         ...
         <div class="wrap push" id="home" style="padding: 0px 0 0;">
         <div class="view-product" id="view">
				 ...
				 <?php $query=$this->db->query("select * from product_general_info where product_id='$product_data->product_id'");
				  			$recd1=$query->row(); ?>
			<div class="cont span_2_of_3 inner" style="padding: 2px 10px;">
			<h3 class="m_3"><?php $prodnm_forgoogle=$recd1->name; echo $recd1->name;?></h3>
				...
				<div class="flexslider">
                		<a  class="backbtn" onclick="backbtn()" style="display:none;">Back</a>
							  <ul class="slides">
								...
								$pord_firstiamgeforgoogle= base_url().'images/product_img/'.$rec[0]; 
							   foreach($rec as $val){ ?>
			...
			<div class="cont1 span_2_of_a1 pull-right">
                          ...
                          <div class="price-dtls">
                              ...
                              <?php $curnt_price=0;
								...
								?>
								<div>
									     	<p>MID-<?php echo $product_data->product_id; ?></p> 
									     </div>
                                <div class="cut-price"tyle="color:black";><b>MRP - Rs.<?=ceil($recd->mrp); ?></b></div>                        
                                <?php if($recd->price != 0){  ?>
								...
								<?php }else{ ?>
                                    ...
                                    <?php if($recd->price != 0){ $curnt_price=$recd->price;?>
                                    	<div>
									     	<p>MID-<?php echo $product_data->product_id; ?></p> 
									     </div>
                                    <div class="cut-price" tyle="color:black";><b>mrp - Rs.<?=ceil($recd->mrp); ?></b></div> 
                                    <div class="actual"><span> Sell Price - Rs.<?=ceil($recd->price); ?><?php $cur_price=ceil($recd->price); ?></span></div>
                                    <?php }else{ $curnt_price=$recd->mrp;?>          	<div>
									     	<p>MID-<?php echo $product_data->product_id; ?></p> 
									     </div>              
                                    <div class="actual"><span style="black;"><b>mrp - Rs.<?=ceil($recd->mrp); ?><?php $cur_price=ceil($recd->mrp); ?></b></span></div> &nbsp;&nbsp;
                                    <?php }?>
								...
								<?php }else{ ?>
                                    ...
                                    <div class="cut-price" style="color:black";><b> MRP - Rs.<?=ceil($recd->mrp); ?></b> </div>
                                   <div class="actual"><span> Sell Price - Rs.<?=ceil($recd->price); ?><?php $cur_price=ceil($recd->mrp); ?></span></div>  
                                    <?php }else{ $curnt_price=$recd->mrp;?>
                                    <div class="actual"><span style="color:black";><b> MRP - Rs.<?=ceil($recd->mrp); ?><?php $cur_price=ceil($recd->mrp); ?></b></span></div>
                                    <?php }?>
 ...
 b on a.seller_id=b.seller_id where b.product_id='$product_data->product_id' and b.sku='$data_sku'");
                                    ...
                                    <?php } else {?>
                                    	<span style="float:left; width:50%">Shipping fee &nbsp;Rs.<?php echo $rew->shipping_fee_amount; ?></span>  
              ...
              <div class="delivry-detls gray-box">
                <h5 style="border-bottom:none;"> Check PinCode & Availability </h5>
                ...
                <ul class="star">
                    ...
                    <li class="add-to-links right">
                        ...
                        <?php } else {?>
                        	<i class="fa fa-heart-o"></i><a href="<?php echo base_url().'user/m_login' ?>" onClick="addtoWishlist_temp(<?=$product_data->product_id; ?>,'<?=$data_sku; ?>')">Add to Wish list</a></span>
                        <?php }?>
                    </li>
                 </ul>
                </div>
                   <div class="options" style="border-bottom:none; border-top:none;">
                   <?php
				   		$query_curclolr=$this->db->query("SELECT color, size,lvl2,lvl2_name,Capacity,RAM,ROM,seller_status FROM cornjob_productsearch WHERE sku='$data_sku' group by sku ");
						...
						$prodcatgnm_forgoogle=$query_curclolr->row()->lvl2_name;
						$prodcolor_forgoogle=$query_curclolr->row()->color;					
            ...
            <button type="button" title="Buy Now" class="buy-btn" onClick="goAddtoCartBuyNow('<?=preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace("'", "",str_replace(" ","-",strtolower($recd1->name)))));?>','<?=$product_data->product_id;?>','<?=$data_sku;?>')" >Buy Now</button> <br/>
                    ...
                    <?php }else{ ?>
                        ...
                        <input type="checkbox" name="ltbn_name" style="display:none;" value="<?php echo $data_sku ?>" id="ltbn_id">
           				<a href="<?php echo base_url().'user/m_login' ?>" onClick="logintobuynow('<?php echo preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace("'", "",str_replace(" ","-",strtolower($recd1->name))))); ?>','<?php echo $product_data->product_id; ?>','<?php echo $data_sku ?>')" class="buy-btn" title="Buy Now" >Buy Now </a><br/>
            <?php } }?>
				</div>
				<!-------------------------------------------Add To Cart And Buy Now Section Start--------------------------------------->    
         ...
         <div class="container-fluid">
    			...
    			<p class="m_desc"><?php $proddescrp_forgoogle= stripslashes($recd1->description); echo str_replace('\\', '', $recd1->description); ?></p>
               </div>
               <!------------------------------------Short Description Section Start-------------------------------------------->
...
<iframe style="padding:4px; margin-top:4px;" width="100%" height="200" src="https://www.youtube.com/embed/<?=$url?>" frameborder="0" allowfullscreen></iframe>
 ...
 <?=ceil($rw['price'])?> </span>
        ...
        <?php } // section 9 condition end ?>
             ...
             <?php }}?>
            <!------------------------------------Backend Section End------------------------------------------------>
            <?php }$this->db->cache_on(); // if $sec_info not false condition end ?> 
			</div>
            </div>
		</div>
  </div>
