...
<?php foreach($cart_data->result() as $rec_cart) { ?>
 ...
 <div class="cart_box">
  ...
  $total_price=0; foreach($rder_query->result() as $rec_cart1){ ?>
   ...
   $qr4=$this->db->query("select name from product_general_info where product_id='$rec_cart1->product_id'");
   $rw4=$qr4->row();
   $qr1=$this->db->query("select imag from product_image  where product_id='$rec_cart1->product_id'");
   $rw1=$qr1->row();
   $image_cart=explode(',',$rw1->imag);
   
    ?> <img src="<?php echo base_url().'images/product_img/'.$image_cart[0]; ?>" width="" class="img-responsive" alt="<?=$rw4->name?>"></div>
   ...
   ?>
    ...
    <div class="actual" > <span> Rs.
	  ...
	  <?php  echo $rec_cart1->sub_total_amount;
	   ...
	   ?> </span> </div>  
    <div class="clearfix"></div>    
  <div class="gray-sml-btn" style="margin:10px 0px;"> <a href="#" onClick="cancel_order_valid('<?php echo $rec_cart1->addtocart_id ?>','<?php echo $rec_cart1->order_id ?>')"> <i class="fa fa-times-circle"> Cancel Order </i> </a> </div>
