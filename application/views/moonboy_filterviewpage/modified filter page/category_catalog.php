...
function FilterProduct_PriceData(cat_name,cat_id,lastseg){
	...
	if(start_price == '' || end_price == ''){
		...
		if(lastseg == 'NOT'){
			window.location.href='<?php echo base_url();?>product_description/category_catalog/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+'price='+price_slab;
		}else{
			window.location.href='<?php echo base_url();?>product_description/category_catalog/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+lastseg+'&price='+price_slab;
		}
	}
}
function sortby_price(sortbyprice,catgnm,cat_id,lastseg)
{
	...
	if(lastseg == 'NOT'){
		window.location.href='<?php echo base_url();?>product_description/category_catalog/'+catgnm.replace(" ","-")+'/'+cat_id+'/'+'sortbyprice='+sortbyprice;
	}else{
		window.location.href='<?php echo base_url();?>product_description/category_catalog/'+catgnm.replace(" ","-")+'/'+cat_id+'/'+lastseg+'&sortbyprice='+sortbyprice;
	}	
}
...
function resetFilterIndivisually(cat_name,cat_id,total_param,reset_param){
	...
	window.location.href='<?php echo base_url();?>product_description/category_catalog_filter/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+res;
}
...
function filter_attribute(cat_name,cat_id,lastseg,attrbtype,attrbvalue,attrbsli){
		...
		window.onload = $('#limg').css('display','block');
		if(lastseg == 'NOT'){
			window.location.href='<?php echo base_url();?>product_description/category_catalog_filter/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+attrbtype+'='+attrbvalue;
		}else{ 
				...
				{ 
					...
					if(lastseg=='')
					{window.location.href='<?php echo base_url();?>product_description/category_catalog_filter/'+cat_name.replace(" ","-")+'/'+cat_id;}
					
				else if(lastseg!='')
				{
					window.location.href='<?php echo base_url();?>product_description/category_catalog_filter/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+lastseg;
				}
			}
			else{
			window.location.href='<?php echo base_url();?>product_description/category_catalog_filter/'+cat_name.replace(" ","-")+'/'+cat_id+'/'+lastseg+'&'+attrbtype+'='+attrbvalue;}
		}
...
function redirectCategoryPage(category_id,sl,cat_name){
	window.location.href='<?php echo base_url();?>product_description/category_catalog/'+cat_name.replace(" ","-")+'/'+category_id;
}
</script>
...
<section  class="filter-form">
					<h4>Categories</h4>
						<div class="row1 scroll-pane">
							...
							$sl++;
							?>
								<?php /*?><label class="radio"><input type="radio" name="radio" onClick="redirectCategoryPage('<?=$res->category_id;?>',<?=$sl;?>,'<?= $res->label_name; ?>')"><i></i><?php echo $res->label_name; ?><?php */?>
                                
                                <label class="radio"><input type="radio" name="radio" onClick="window.location.href='<?php echo base_url().'product_description/product_addtocart/'.preg_replace('#/#',"-",str_replace(" ","-",strtolower($res->label_name))).'/'.$res->category_id ?>'"><i></i><?php echo $res->label_name; ?>
                                </label>
                                <?php } ?>
							</div>
		  ...
		  <div class="grid1_of_4 catlg">
               ...
               <div class="view view-fifth">
                ...
                <a href="<?php echo base_url().'product_description/product_detail/'.preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" >
    				...
    				<img src="<?php echo base_url();?>images/product_img/prdct-no-img.png" class="img-responsive" alt="no image">
    				<?php }else{?>
		...
		<?php /*?><div class="modal fade" id="myModal<?=$sl?>" role="dialog" aria-labelledby="myModalLabel"> 
          ...
          <?php } // End of product data is not false condition ?>
           ...
           <div id="view_more_dv">
				...
				<input type="button" class="add-to-cart view_mor" value="view more" name="button" onClick="ShowMoreData('<?=$no_of_product;?>','<?=$this->uri->segment(4);?>','<?php if($this->uri->segment(5) != ''){ echo $this->uri->segment(5);}else{ echo 'NOT';}?>')">
				<?php }	?>
           </div>
</div>
</div>
...
.rstClear{ background-color:#ccc; padding:5px 5px;  border-radius:2px;}
