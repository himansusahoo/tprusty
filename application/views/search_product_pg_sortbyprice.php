...
function sortby_price(sortbyprice,title)
{
	window.onload = $('#limg').css('display','block');	
	
	$.ajax({
		url:'<?php echo base_url(); ?>product_description/product_search',
		method:'get',
		data:{sortbyprice:sortbyprice,title:title},		
		success:function(result){	
          ...
          <h4>Categories</h4>
              <div class="row1 scroll-pane">
				  ...
				  if($product_data->num_rows() > 0){
					  foreach($product_data->result() as $cat_row){
				  ...
				  ?>
                        ...
                        <label class="radio"><input type="radio" name="radio" onClick="redirectCategoryPage('<?=$cat_name_rw->category_id;?>','<?= preg_replace('#/#',"-",str_replace(' ','-',$cat_name_rw->category_name)); ?>')"><i></i><?php echo $cat_name_rw->category_name; ?></label>
		...
		<div class="catlog">
			...
			<!--<ul class="f_nav">
             ...
             <div class="dropdown_left">
			       Sort By Price &nbsp;<select class="dropdown selectpicker" id="attr_size"  data-style="btn-info" style="width:auto;" onChange="sortby_price(this.value,'<?=$this->input->get('search');?>')" >        <option value=''>--Select--</option>
		  ...
		  <div class="grid1_of_4 catlg">
               ...
               <div class="view view-fifth">
                ...
                <a href="<?php echo base_url().'product_description/product_detail/'.preg_replace('#"#',"-",preg_replace('#/#',"-",str_replace(' ','-',strtolower($rw->name)))).'/'.$rw->product_id.'/'.$rw->sku  ?>" >
    				...
    				<?php }else{?>
					<img src="<?php echo base_url();?>images/product_img/<?=$image_arr[0];?>" onerror="imgError(this);" class="wow flipInY grow" data-wow-delay="1s" alt="">
   					<?php } ?>
