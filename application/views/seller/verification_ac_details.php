...
<html lang="en-US">
    ...
    <script>
        ...
        $("#verify_submit_id").prop("disabled", true).css({'b ac kg ro und-color': '#ccc', 'border': '1px solid #ccc'});
                $(sell_online_input).attr("disab le d", "dis ab led");
                $('#item_shipping_ true').c lick(function () {        $('#sell_online_input').removeAttr("disabled");
        });
    ...
    <script type="text/javascript">
                 ...
                 */
                        ...
                        var base_url = "<?php echo base_url(); ?>";
                                varcont r oller = " s   eller/sel l er ";
                                var seller _ id = $('#sessi o n_seller_id').val();
                ...
                / * On 26/10 / 15
                        ...
                        var base_url = "<?php echo base_url(); ?>";
                                ...
                                'url' : base_url + controller + '/send_otp',
                                        'type': 'POST',
                                        'data' : 'mobile=' + mobile,
                                        //'s uccess' : fu nction(data){
                                        //$('#successfully_verify').html(data);
                                        //docu ment .getElementById("review_from").reset();
                                        //}          });
                                } * /

                                                function email_verific ation() {
                                        var base_ url = "<?php echo base_url(); ?>";
                                        var controlle r = "seller/seller";
                                        var emai l = $('#email').val();
                                ...
                                f uncti on submit_opt() {             var base_url = "<?php echo base_url(); ?>";
                                        var co ntroller = "seller/seller";
                                        var otp = $('#otp').val();
                                        var email_id = $('#myDiv').val(); //alert(email_id); return false;
                                        if (otp == "") {
                                $("#otp_validate").show();
                                        $(" #otp _validate").focus().text("Please enter correct OTP.");
                                        return false;
                                } else {
                                $.ajax({
                                'url': base_url + controller + '/otp_verification',
                                        ...
                                        $.colorbox.close();
                                                //$('#succes s fully_verify').html(data);
                                                if (data == 'success') {
                                        $("#verify_submit_id").prop('disabled', false).cs s ({'background-color': '#000', 'bo rder': '1px solid #000'});
                                                $("#number_button").prop('disabled', true);
                                                $('#successfully_verify').html("<div style='color:green;'><img src='<?php echo base_url(); ?>images/tick.png' width='15'>  Verify successfully.</div>"); } else {
                                        $("#otp").val("");
                                                $('#succ e s sf ully_verify').html("<div style='color :red;'><img src='<?php echo base_url(); ?>images/cross.png' wid th='15'>  Verification Failed.</div>");
                                        }
                                        }
                                });
                                }
