...
<script>
	$(document).ready(function(){
		...
		$('input[name="productStatus[]"]').click(function(){ 
			...
			if (val){
				$.ajax({
					'url' : base_url + controller + 'filter_exist_product_status',
					'type' : 'POST',
					'data' : 'approve_status='+val,
					'success' : function(data){
						$('#filtered_exit_product_table').html(data);
					}
				});
			}else{
				...
				<div class="main-content">
					...
					<div class="row gray_bg">
						...
						<div class="col-md-10 mt20">
							<div class="a-center" id="loader_image" style="display:none;">
								<img src="<?php echo base_url();?>images/loader.gif">
							</div>
							<div id="successfully_verify"></div>
							<!--<ul class="nav nav-tabs">
								<li class="active"><a data-toggle="tab" href="#tab1"> Live </a></li>
								<li><a data-toggle="tab" href="#tab2"> Non Live </a></li>
								<li class="no_tab">
									<div class="right order_id_search">
										<img class="partner_img" src="<?//php echo base_url();?>images/partner-mobile-icon-new.png">
										<a href="#"> Cataloging Services </a>
										<div class="search_bar input-append">
											<input type="text" name="order_id" placeholder="Search SKU ID/FSN">
										</div>
									</div>
									<div class="clear"></div>
								</li>
									...
									<div class="row">
										<div class="col-md-12">
                                            ...
                                            <div class="search_field_dv">
                            					...
                            					echo form_open_multipart('seller/catalog/search_exits_prdt_list_slr', $attributes);
                            					?>
											...
											<table class="table neworder_table" id="filtered_exit_product_table" style="background-color:#fff;">
...
<?php 
	if($exist_newProduct_list) {
		$sl=0;
	foreach ($exist_newProduct_list as $row):
	$sl++;
	//var_dump($row);exit;
	$img = $row->imag; 
	$image = explode(',', $img);  //echo $image[0]; exit;
	$first_img = $image[0];
...
?>
												<tr>
														...
														<div class="row neworder_product_block">
															<div class="col-md-12">
																<div class="left position_relative image_block">
                                                                	<?php
                                                                    if($row->approve_status == 'Active'){
																		//condition for if seller disabled the product
																		if($row->status=='Disabled'){
																	?>
                                                                    <img src="<?php echo base_url().'images/product_img/catalog_'.$first_img; ?>" onClick="alert('This product is Disabled \n So, you can not view in frontend.');" width="65">
                                                                    <?php }else{ ?>
                                                                    ...
                                                                    <img src="<?php echo base_url().'images/product_img/catalog_'.$first_img; ?>" width="65"></a>
                                                                    
                                                                    <?php } }else{ ?>
                                                                    <img src="<?php echo base_url().'images/product_img/catalog_'.$first_img; ?>" onClick="alert('This product is not Active \n So, you can not view in frontend.');" width="65">
                                                                    <?php } ?>
																</div>
																	...
																	<div>
																		<span class="block">
                                                                            ...
                                                                            <?php }else{ ?>
                                                                             ...
                                                                             <?php if($row->approve_status=='Suspended' || $row->approve_status=='Pending' )  {?> 
                                                                             <img src="<?php echo base_url();?>images/block.png " width="15" height="15"> 
																			 <?php }?>
													...
													<td><?=$row->status?></td>
													<td>
														<a class='inline' href="#inline_content<?=$row->seller_exist_product_id?>" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil-square-o" style="font-size:25px;"></i></a>
													</td>
                                                    <td>
                                                    <?php
                                                    //program start for getting product price//
													if($row->special_pric_to_dt >= $cdate && $row->special_price != 0){
                                                            ...
                                                            <div class="col-md-12">
                                                            	<table class="table">
                                                                	<!--<tr>
                                                                        ...
                                                                        <td>
                                                                        	<input type="text" size="10px" name="selleing_price" value="<?//=$final_price;?>">&nbsp;&nbsp;&nbsp;&nbsp;
...
?>							
							<div style="display:none">
								<div id='inline_content<?=$row->seller_exist_product_id?>' style='padding:0 10px; background:#fff;'>
										...
										<div class="edit_form_content">
											<div class="row">
												<div class="col-md-4 right_border">
													<div><img src="<?php echo base_url().'images/product_img/catalog_'.$first_img; ?>" width="65"></div>
													...
													<table width="100%" border="0" cellspacing="5">
														<tr>
															<td>
																...
																<input type="hidden" id="hidden_id<?=$sl; ?>" value="<?=$row->seller_exist_product_id?>">
															</td>
															<td>
														...
														<tr>
															<td>
																...
																<input name="price_from_date" id="datepicker-example15-start<?=$sl?>" class="seller_input NEW_FROM_DATE" type="text" value="<?=$row->special_pric_from_dt?>">
															</td>
															<td>
																<label>Special Price To Date : </label><br>
																<input name="price_to_date" id="datepicker-example15-end<?=$sl?>" class="seller_input NEW_TO_DATE" type="text" value="<?=$row->special_pric_to_dt?>">
															</td>
														</tr>
                                                        <tr>
															<td>
                                                            ...
                                                            <td>
																...
																<input type="text" name="weight" id="prdt_weight<?=$sl?>" value="<?=$row->weight?>" class="seller_input readonly_field">
															</td>
														</tr>
														<tr>
															<td> 
														...
														<tr>
															<td>
																<input type="button" name="submit" class="seller_buttons" onClick="validate_form(<?=$sl; ?>, <?=$row->seller_exist_product_id?>, '<?=$row->approve_status?>','<?=$row->master_product_id?>','<?=$row->sku?>')" value="Save">
															</td>
														</tr>
													</table><div class="loader_div_seller" style="text-align:center;"> <img src="<?php echo base_url().'images/loading1.gif' ?>" /></div>
												</div>
											</div>
										</div>
									</div>
...
<script>
	function validate_form(sl, seller_exist_product_id, approve_status,master_prcdt_id,sku){
		var base_url = "<?php echo base_url(); ?>";
		var controller = "seller/catalog/";
		...
		if(status == ""){ 
			...
			$("#selling_price"+sl).focus().css('border-color','red');
			...
			$(".validate_msg").show().text('Selling price should be an Integer value.');
			return false;
			...
			$("#shipping_typ"+sl).css('border-color','#ccc');
			$("#default_shipng_fee"+sl).css('border-color','red').select();
			$(".validate_msg").show().text('Shipping Fee Invalid.');
			return false;
		}/*else if(shipping_typ == "Free" && default_shipng_fee != ""){
			$("#product_status"+sl).css('border-color','#ccc');
			$("#selling_price"+sl).css('border-color','#ccc');
			$("#special_price"+sl).css('border-color','#ccc');
			$("#qty"+sl).css('border-color','#ccc');
			...
			$("#shipping_typ"+sl).css('border-color','#ccc');
			$("#shipping_typ"+sl).css('border-color','red');
			$(".validate_msg").show().text('To set Shipping fee choose Default shipping type.');
			return false;
		}*/else{ 
			$("#product_status"+sl).css('border-color','#ccc');
			$("#selling_price"+sl).css('border-color','#ccc');
			$("#special_price"+sl).css('border-color','#ccc');
			$("#qty"+sl).select().css('border-color','#ccc');
			$(".validate_msg").hide();
		
		$('.loader_div_seller').css('display','block');
			$.ajax({
				url : base_url + controller + 'update_exist_new_product',
				type : 'POST',
				data : {status:status,price:price,selling_price:selling_price, special_price:special_price,special_price_fr_date:special_price_fr_date,special_price_to_date:special_price_to_date, qty:qty, seller_exist_product_id:seller_exist_product_id, approve_status:approve_status,master_prcdt_id:master_prcdt_id,sku:sku,vat_cst:vat_cst,shipping_typ:shipping_typ,default_shipng_fee:default_shipng_fee,hidden_shipping_fee:hidden_shipping_fee},
				success : function(data){
					//if(data == 'success'){
...
function ShowCalculateDv(sl,third_leable_cat_id,final_price,service_tax,shipping_fee){
	...
	//alert(third_leable_cat_id);exit;
	$.ajax({
