...
if($rows > 0){ 
	foreach($filtered_data->result() as $row) { 
		...
		echo "<tr>
				...
				<td>".$row->status."</td>
				<td><a class='inline' href='#inline_content".$row->seller_exist_product_id."' data-toggle='tooltip' title='Edit'>
...
<script>
	/*function validate_form(sl, seller_exist_product_id, approve_status){ 
		...
		if(status == ""){ 
			...
			$("#price"+sl).focus().css('border-color','red');
			$(".validate_msg").show().text('Product Price is required.');
			return false;
		}else if(isNaN(price)){
			$("#product_status"+sl).css('border-color','#ccc');
			$("#price"+sl).select().css('border-color','red');
			$(".validate_msg").show().text('Price should be an Integer value.');
			return false;
		}else if(special_price == ""){
			$("#product_status"+sl).css('border-color','#ccc');
			$("#price"+sl).css('border-color','#ccc');
			...
			$.ajax({
				...
				type : 'POST',
				data : {status:status, special_price:special_price, qty:qty, seller_exist_product_id:seller_exist_product_id, approve_status:approve_status},
