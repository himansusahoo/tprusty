					...
					<div>
						<span class="input-append">
							...
							<div class="btn-group" data-toggle="buttons-radio">
								<button class="btn active" onClick="goSetteledTrans()" type="button">Settled Transactions</button>
								<button class="btn" style="background-color:#aaa;" type="button">Unsettled Transactions</button>
							</div>
						</span>
						<!--Take tour-->
						<!--<span>
					...
					<div class="row mt20 settelment_period">
						<form>
							<div class="input-append">
								<label id="settelment_period_label">Due Date : &nbsp </label>
								<div id="settelment_period_date">
									...
									<input type="button" class="seller_buttons" value="Search" onclick="unsettledTransBtnTwoDates()">
								</div>
							</div>
...
?>
							<tr id="searched_transaction">
								...
								<td><?=$row->FINAL_SETTELED_AMT;?></td>
								<td>
                                <?php
								if($row->product_order_status == 'Delivered'){
									$due_date= date('Y-m-d', strtotime($row->order_status_modified_date. ' + 3 days'));
									echo $due_date;
									
								}else{
									echo 'yet not delivered';
								}
								?>
                                </td>
							</tr>
...
<script>
	function unsettledTransBtnTwoDates(){
		...
		if(from == ""){
			...
			window.location.href = '<?php echo base_url();?>seller/payments/searchUnsetteldTransByDates/'+Base64.encode(from)+'&'+Base64.encode(to);
		}
	}
</script>

<script>
function goSetteledTrans(){
	window.location.href='<?php echo base_url();?>seller/payments/transactions';
}
</script>
<?php
