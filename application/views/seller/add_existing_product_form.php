...
date_default_timezone_set('Asia/Calcutta');
	...
	echo form_open_multipart('seller/catalog/save_exist_new_product');
?>
			<div class="row">
					...
					<table width="100%" border="0" cellspacing="5">
						...
						<tr>
							<td>
								...
								<div id="shipping_fee_dv">
									Local : <input type="text" class="text dt" name="local_shipng_fee" id="local_shipng_fee">[INR] <br>
									Zonal : <input type="text" class="text dt" name="zonal_shipng_fee" id="zonal_shipng_fee">[INR] <br>
									National : <input type="text" class="text dt" name="national_shipng_fee" id="national_shipng_fee">[INR]
								</div>
							...
							<td>
								Maximum Qty Allowed in Shopping Cart : <br>
								<input type="text" name="max_qty_allowed" class="seller_input">
							</td>
						</tr>
						<tr>
							<td>
								Enable Qty Increments : <br>
...
deleteCallback: function (data, pd) {
    for (var i = 0; i < data.length; i++) {
		//var seller_id = "<?php /*?><?php echo $this->uri->segment(4); ?><?php */?>";
...
downloadCallback:function(filename,pd)
	{
		location.href="<?php echo base_url();?>seller/catalog/download_product_tmp_image?filename="+filename;
	}
});  
});
</script>
...
<script>
	function Valid_exist_product_form(){
		...
		var re = /^[A-Za-z0-9_-]*$/
		var prod_type = $("select[name='prod_type'] option:selected").val(); //alert(status);return false;
		if(prod_type == ""){
			$('.valid_msg_dv').show().text('Product Type is required.');
			$("select[name='prod_type']").css('border-color','red').focus();
			return false;
		}
		var sku = $("input[name='sku']").val(); 
		
		if(sku == ""){
			$("select[name='prod_type']").css('border-color','#ccc');
