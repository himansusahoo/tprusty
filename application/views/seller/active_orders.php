...
?>
									...
									if($rom != ''){ echo "<span class='cart_attr'>ROM : ".$rom.'</span><br/>'; }
                                            ...
                                            ?>                                    
														...
														<td><?php if($row->order_status=='Pending payment'){ echo "Pending"; } else { echo $row->product_order_status; } ?><br> <?php if($row->order_confirm_for_seller=='Not Approved' or $row->order_status=='Pending payment' or $row->order_status=='Failed' ) { ?> <span style="color:#F00;"><?php echo 'Orders cannot be ship until confirmed' ; }?></span></td>
														<td>
															<div>
...
//														}
                                                        ...
                                                        <?php
														if($row_as_orderid->grace_period_approve_status=='Not Approved' && $row_as_orderid->grace_period==0 
														&& 	$row_as_orderid->request_for_grace_period=='yes' && $row_as_orderid->grace_period_reason!='' )
														{
