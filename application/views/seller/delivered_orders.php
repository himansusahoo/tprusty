			...
			<div id="content">    
				...
				<div class="main-content">
					...
					<div class="page_header">
						<div class="left">
							<h3>Delivered Orders <span id="ajxtst"></span></h3>
						</div>
						...
						<div id="tab2" class="tab-pane fade in active">
							<div class="row track_orders_nav">
								<div class="around_float inner_navigation overflow_hidden">
									<div class="inner_navigation_block left" id="trackOrder1">
										<a href="<?php echo base_url().'seller/Orders/intransist_orders' ?>"><span class="block block_small" >In Transit</span></a>
									</div>
									<div class="inner_navigation_block left selected_block" id="trackOrder2">
							...
							<div class="row">
								...
								<div class="row" id="In_transit" style="display:none;">
									<div class="col-md-12">
                                    <p>
...
//																				echo date_format($date, 'M d, Y h:i A');
												...
												<td><?php /*?><?=$row1->order_status?><?php */?></td>
												<td>
													<div>
														<strong class="block">Qty : <?php /*?><?=$row1->quantity?><?php */?></strong><br>
														<span class="muted">
															Price :
															<i class="icon-rupee"></i>
															<?php /*?><?=($row1->sub_total_amount-$row1->sub_shipping_fees)/$row1->quantity?><?php */?> each
														</span><br>
														<span class="muted">
															Shipping : 
															<i class="icon-rupee"></i>
															<?php /*?><?=$row1->sub_shipping_fees/$row1->quantity;?><?php */?> each
														</span><br>
														<strong>
															Total :
															<i class="icon-rupee"></i>
...
//										$date_despatch_by = new DateTime(date('y-m-d H:i:s' ,strtotime($row1->order_confirm_for_seller_date .'+'.$dispatch_days .'day')));
												...
												<td>
													<?php /*?><?=$row1->full_name?><?php */?>
													<br>
												<?php /*?>	<?=$row1->address?><?php */?>
													<br>
													<?php /*?><?=$row1->city?><?php */?>
													<br>
													<?php /*?><?=$row1->state?><?php */?>
													<br>
													<?php /*?><?=$row1->country?> - <?=$row1->pin_code?><br><?php */?>
													Mobile -<?php /*?> <?=$row1->phone?><?php */?>
												</td>
											</tr>
<?php 
	//endforeach;
...
if($delivered_orders1){
	foreach($delivered_orders1 as $row) {

		
		$query = $this->db->query("SELECT a.date_of_order,a.order_status,a.order_status_modified_date,b.order_id,a.order_id_payment_gateway,b.quantity,b.sub_total_amount,b.sub_shipping_fees,b.sku,c.name,c.imag,c.name,g.*,i.payment_type
		FROM order_info a
		...
		WHERE a.order_id='$row->order_id' AND c.sku='$row->sku' ");
		
	
	$delivered_orders=$query->result();	
															...
															<div class="left details_block">
																<div>
																	...
																	<strong>
                                                                        ...
                                                                        <?php        
                                                                                $color_sizecronjobquery=$this->db->query("SELECT color,size,Capacity,RAM,ROM FROM  cornjob_productsearch WHERE sku='$row2->sku' group by sku ");
												...
												<td>
													<div>
														...
														<span class="muted">
															Price :
															<i class="icon-rupee"></i>
															<?=$row2->sub_total_amount/$row2->quantity?> each
														</span><br>
														<span class="muted">
															Shipping : 
															<i class="icon-rupee"></i>
															<?=$row2->sub_shipping_fees/$row2->quantity?> each
														</span><br>
														<strong>
															Total :
															<i class="icon-rupee"></i>
															<?php echo $row2->sub_total_amount ?>
														</strong>
														<div>
															<span>
																<!--<strong>Payment Type :</strong>-->
