	   ...
	   INNER JOIN product_master e on d.product_id=e.product_id 
		WHERE  a.sku='$rec_cart->sku' ");
                            ...
                            <tr>
                                ...
                                <td> <div class="checkout-img">  
                                    ...
                                    <div class="chckout-desc"> 
                                        <?php
                                        $qr2 = $this->db->query("select name from product_general_info where product_id='$rw1->product_id'");
                                        $rw2 = $qr2->row();
                                    ...
                                    <?php
                                    if ($avail_qnt <= 0 || $avail_status == 'Disabled' || $avail_approve_status == 'Inactive' || $seller_status != 'Active') {
                                        if ($avail_qnt <= 0) {
                                            ?> <span style="background-color:#900; color:#FFF; font-weight:bold;">This Product is currently Out Of Stock. </span> <?php } else { ?>
			...
			WHERE a.seller_id = '$rw_sellername->seller_id'");
                            ...
                            if ($avail_qnt > 0 && $avail_status == 'Enabled' && $avail_approve_status == 'Active' && $seller_status == 'Active') {
        ...
        <html lang="en">
            ...
            <!--<ul class="back"> <li><i class="back_arrow"> </i>Back to <a href="index.html">Men's Clothing</a></li> </ul>-->
                ...
                <div  class="checkout" >
                    <h3 class="title3" style="text-transform:capitalize;"> Your Cart </h3>
                    <?php
                    //$i=0; $sku_arr=array(); foreach($sku as $key=>$rec_cart){ $sku_arr[$i]= $rec_cart; $i++; }   $j=0 ;
                    //print_r($sku);exit;
                    $arr = (array) $sku;
                    if (empty($arr)) {
                        ?>
...
//		WHERE  a.sku='$rec_cart->sku'  AND c.sku='$rec_cart->sku' ");
	   ...
	   INNER JOIN product_master e on d.product_id=e.product_id 
		WHERE  a.sku='$rec_cart->sku' ");
                                        ...
                                        <div class="chckout-desc"> 
                                            <?php
                                            $qr2 = $this->db->query("select a.name from product_general_info a inner join product_master b on a.product_id=b.product_id  where b.sku='$rec_cart->sku'");
                                            $rw2 = $qr2->row();
                                    ...
                                    <td>
                                        <?php
                                        $qr2 = $this->db->query("select a.description from product_general_info a inner join product_master b on a.product_id=b.product_id  where b.sku='$rec_cart->sku'");
                                        $rw2 = $qr2->row();
