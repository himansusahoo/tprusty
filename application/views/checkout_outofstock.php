...
<div class="line">&nbsp;</div>
  ...
  <tr>
   ...
   $qr1=$this->db->query("select a.imag,b.sku,c.name,b.quantity,b.product_id from product_image a inner join product_master b on a.product_id=b.product_id inner join product_general_info c on b.product_id=c.product_id where a.product_id='$rec_cart->product_id'");
   $rw1=$qr1->row();
  ...
  echo "<h3  class='product-name'>"."<a href="."'".base_url().'product_description/product_detail/'.preg_replace('#/#',"-",str_replace(" ","-",strtolower($rw1->name)))."/".$rw1->product_id."/".$rw1->sku."'"." target=_blank>". $rw2->name ."</a>"."</h3> " ;
	...
	if($avail_qnt>0){ array_push($color_arr,$color_val);}
	...
	if($avail_qnt>0){array_push($size_arr,$size_val);}
   ...
   if($avail_qnt>0){array_push($seller_id_arr,$seller_id_arr_row->seller_id); }//seller_id array
...
//    print_r($rw_sellername->business_name);}
  ...
  <?php /*?> <a onClick="gosellerReview(<?= $rw_sellername->seller_id; ?>)" id="goslr" style="cursor:pointer !important;"><?php */?>
   ...
   <!--  <div class="fulfill"> <img src="<?//php echo base_url()?>images/moon-fulfilled.png"  alt="">  </div>-->
      ...
      <?php if($avail_qnt<=0) {?> <span style="background-color:#900; color:#FFF; font-weight:bold;">This Product is currently Out Of Stock, You Cannot Buy. </span> <?php } ?>
          <div class="clearfix"> &nbsp;</div>
   ...
   */
   ...
   $rec_ct=$qr2->num_rows(); 
   echo  $rec_ct;
  if($avail_qnt>0){ array_push($qantity_arr,$rec_ct);}
   ?>
    </td>
    <td  align="center">  
    <h4> Rs. <?php  $user_id=$this->session->userdata['session_data']['user_id'];
		 ...
		 $tax_amount = $rec4[0]->price*$taxdecimal;
		  ...
		  if($rec4[0]->special_price !=0){
			  if($cdate >= $special_price_from_dt && $cdate <= $special_price_to_dt){
				 if($avail_qnt>0){ array_push($tax_arr,$tax_amount_special*$rec_ct); }
				  ...
				  //array_push($tax_arr,$tax_amount*$rec_ct);
					...
					if($rec4[0]->price != 0){
						if($avail_qnt>0){array_push($tax_arr,$tax_amount*$rec_ct);}
				    	$price= $price + $rec4[0]->price;
					}else{
						if($avail_qnt>0){array_push($tax_arr,$tax_amount_mrp*$rec_ct);}
	  ...
	  if($avail_qnt>0){ array_push($price_arr,$final_price);}
	...
	if($avail_qnt>0){array_push($shipping_fees_arr,$rec4[0]->shipping_fee_amount*$rec_ct); }
    ...
    <span class="price-blue"> 
	  ...
	  if($avail_qnt>0){array_push($sub_total_arr,$subtotal_price);}
      ?>
   </span><br>
  ...
  </tr><?php if($avail_qnt>0){$total_price=ceil($total_price+$subtotal_price); }
  
 if($avail_qnt>0){ array_push($addtocart_id_arr,$rec_cart->addtocart_id);}
 // echo $rec_cart->sku;
 if($avail_qnt>0){ array_push($sku_arr,$rec_cart->sku);}
  } ?>
...
<li><a id="cod_link" onClick="pay_by_cod(); DrawCaptcha();" style="cursor:pointer;" > <i class="fa fa-inr"></i> COD </a></li>
</ul>
</div>
