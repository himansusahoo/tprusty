...
class Product_description extends CI_Controller {
	...
	function product_detail()
	{
		...
		//-------------------sujit start----------------------------//
		$this->db->cache_off();
		$cachequery=$this->db->query("SELECT * FROM `cornjob_productsearch` WHERE `sku`='$sku_id' ");
		$cachecatgid_id=$cachequery->row()->lvl2;
		
		
		
		$query=$this->db->query("select clear_catch.sql_id as clear_catchID ,prod_cache.sql_id as prod_cacheID,clear_catch.catg_id,clear_catch.folder_name from clear_catch inner join prod_cache on clear_catch.sku=prod_cache.sku where prod_cache.catg_id in ($cachecatgid_id)");
		$cnt=$query->num_rows();
		if($query->num_rows()>0)
		{
			foreach($query->result_array() as $res)  
			{
				$catg_id=$res['catg_id'];
				$clear_catchID=$res['clear_catchID'];
				$prod_cacheID=$res['prod_cacheID'];
				//echo $catg_id;exit;
				$folder_name=$res['folder_name'];
				list($folder_name1, $folder_name2) = explode("+", $folder_name, 2);
				$this->db->cache_delete($folder_name1, $folder_name2);
				$query=$this->db->query("DELETE FROM clear_catch WHERE sql_id='$clear_catchID'");
				$query=$this->db->query("DELETE FROM prod_cache WHERE sql_id='$prod_cacheID'");
			}
		}
		$this->db->cache_on();
		...
		$this->Product_descrp_model->catch_data_insert_for_single_prod($cachecatgid_id,$sku_id);
		//-------------------sujit end----------------------------//
