...
//       </div>";
	...
	function changed_password(){
		...
		if($result == true){
			...
			if($result1 == true){
				...
				");
				if($this->email->send()){
					echo 'changed_success';exit;
				}else{
					echo 'invalid_pass';exit;
				}
			}
		}else{
			echo 'invalid_pass';exit;
	...
	function add_wishlist_temp(){
		$result = $this->Usermodel->insert_inn_wishlist_temp();
		if($result == true){
			echo 'success';exit;
		}else{
			echo 'exists';exit;
	...
	function order_details(){
		if($this->session->userdata['session_data']['user_id']){
			...
			if($this->agent->is_mobile())
			{$this->load->view('m/order_details',$data);}else
