...
class Seller extends CI_Controller {
	...
	function forgot_passord(){
			...
			if($result != false){
				...
				/*$this->email->message("
					<html>
					<head>
						<title> Moonboy Seller Forgot password </title>
					</head>
					<body>
	...
	/* On 26/10/15
	function loadForm(){
		$validation_type = $this->input->post('validation_type'); 
		if($validation_type == 'email'){ 
			$this->load->view('seller/email_validation_form');
		}else if($validation_type == 'mobile'){
			$this->load->view('seller/mobile_validation_form');
		}
	}
	*/
	...
	function send_email_code(){
		if($this->session->userdata('seller-session')){
			...
			/*$this->email->message("
				<html>
					<head>
						<title> Moonboy Seller Account Verification </title>
					</head>
					<body>
						<div style='width:50%; margin:0px auto; padding:40px;  background-color:#f4f4f4; border:10px solid #ef3038;'>
							<p> Dear Seller </p>
							<p> Greetings from Moonboy Marketplace! </p>
							<p> Thank you for signing up with us. Being one of India's largest managed marketplace with more than 4.5 crore registered users, Moonboy gives you access to a large buyer base. Thanks to Moonboy's brand recognition and trust you can enjoy instant credibility just by being associated with us. </p>
	...
	function send_otp(){
		...
		$message = "Dear Seller. Thank you for signing up with moonboy.in. Your Mobile Verification Code : $code"; 
		$str = urlencode("$message");
		$url = "http://mysms.msgclub.net/rest/services/sendSMS/sendGroupSms?AUTH_KEY=839a8be6618222bd1352ab6a3eef5&message=$str&senderId=MONBOY&routeId=3&mobileNos=$mobile&smsContentType=english";
		$data = file_get_contents($url);
		header("#");
		if($data){
			$result = $this->Seller_model->insert_email_code($mobile, $code);
		}
	}
	*/
	...
	function add_category(){
		if($this->session->userdata('seller-session')){
			...
			if($result == true){
				...
				//$this->load->view('seller/seller_information');
			}else{
	...
	/*function cate_verification(){
		$category = urldecode($this->uri->segment(4)); 
		$seller_id = $this->session->userdata('seller-session');
		
		$result = $this->Seller_model->match_category($seller_id, $category);
		if($result == true){
			$this->load->view('seller/seller_information');
		}else{
			$data['error'] = "Incorrect Category !";
	...
	function add_seller_information(){
		if($this->session->userdata('seller-session')){
			...
			if (empty($_FILES['pan_img']['name'])){
				$this->form_validation->set_rules('pan_img', 'PAN Image', 'required');
			}
			$this->form_validation->set_rules('tin', 'TIN', 'required');
			if (empty($_FILES['tin_img']['name'])){
				$this->form_validation->set_rules('tin_img', 'TIN Image', 'required');
			}
			/* Comment on 26/10/15
			$this->form_validation->set_rules('tan', 'TAN', 'required');
			if (empty($_FILES['tan_img']['name'])){
				$this->form_validation->set_rules('tan_img', 'TAN Image', 'required');
			}
			*/
			/*if($_POST['tan']){
				$this->form_validation->set_rules('tan_img', 'TAN Image', 'required');
			}
			$this->form_validation->set_rules('ac_holder_name', 'Account Holder Name', 'required');
			$this->form_validation->set_rules('ac_number', 'Account Number', 'required|numeric');
			$this->form_validation->set_rules('cnf_ac_number', 'Confirm Account Number', 'required|numeric|matches[ac_number]');
			$this->form_validation->set_message('matches', 'Account number should match with Confirm account number.');
			$this->form_validation->set_rules('ifsc', 'IFSC Code', 'required');
			$this->form_validation->set_rules('display_name', 'Display Name', 'required');
			$this->form_validation->set_rules('store_desc', 'Store Description', 'required');
			*/
			//if($this->form_validation->run() != false){  
...
//---------------------------------------sujit end---------------------------------------//								
				...
				//echo $pan_img_name." ".$tin_img_name." ".$tan_img_name." ".$address_img_name." ".$ID_img_name." ".$Cheque_img_name; exit;
				/*$image_name_data = array(
					'pan_img' => $data['pan_img_upload_data']['file_name'],
					'tin_img' => $data['tin_img_upload_data']['file_name'],
					'tan_img' => $data['tan_img_upload_data']['file_name'],
					'address_img' => $data['address_img_upload_data']['file_name'],
					'ID_img' => $data['ID_img_upload_data']['file_name'],
					'Cheque_img' => $data['Cheque_img_upload_data']['file_name'],
				); 
				var_dump($image_name_data); exit;
				
				/*
				$data = array(
					'pan_img_upload_data' => $this->upload->data('pan_img'),
					'tin_img_upload_data' => $this->upload->data('tin_img'),
					'tan_img_upload_data' => $this->upload->data('tan_img'),
					'address_img_upload_data' => $this->upload->data('address_img'),
					'ID_img_upload_data' => $this->upload->data('ID_img'),
					'Cheque_img_upload_data' => $this->upload->data('Cheque_img'),
				);
				echo $data['pan_img_upload_data']['file_name']; exit;
				$name_array = array(
					'pan_img_name' => $data['pan_img_upload_data']['file_name'],
					'tin_img_name' => $data['tin_img_upload_data']['file_name'],
					'tan_img_name' => $data['tan_img_upload_data']['file_name'],
