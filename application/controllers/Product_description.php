...
class Product_description extends CI_Controller {
	...
	function product_data(){
		...
		{$this->load->view('m/subcategory_detail',$p);}else
	...
	function product_addtocart()
	{
		...
		//-------------------sujit start----------------------------//
		/*$this->db->cache_off();
		$query=$this->db->query("select clear_catch.catg_id,clear_catch.folder_name from clear_catch inner join prod_cache on clear_catch.catg_id=prod_cache.catg_id where prod_cache.catg_id in ($dskcatgid_id)");
		$cnt=$query->num_rows();
		if($query->num_rows()>0)
		{
		...
		$this->db->cache_on();
		//-------------------sujit end----------------------------//
		//$this->Product_descrp_model->catch_data_insert($dskcatgid_id);
	...
	function product_addtocart_filter()
	{
		...
		//-------------------sujit start----------------------------//
		/*$this->db->cache_off();
		$query=$this->db->query("select clear_catch.catg_id,clear_catch.folder_name from clear_catch inner join prod_cache on clear_catch.catg_id=prod_cache.catg_id where prod_cache.catg_id in ($catg_id)");
		$cnt=$query->num_rows();
		if($query->num_rows()>0)
		{
			foreach($query->result_array() as $res)  
		...
		$this->db->cache_on();
		//-------------------sujit end----------------------------//
		//$this->Product_descrp_model->catch_data_insert($catg_id);
	...
	function category_catalog()
	{
		...
		//sujit start for category_catalog_cacheclear
		//$this->category_catalog_cacheclear($main_catgid);
		//sujit end for category_catalog_cacheclear see line no 390
		if ($this->agent->is_mobile())
		{
			/*$data['cat_id']=$main_catgid;
	...
	{
		/*$this->db->cache_off();
		$query=$this->db->query("select clear_catch.catg_id,clear_catch.folder_name from clear_catch inner join prod_cache on clear_catch.catg_id=prod_cache.catg_id where prod_cache.catg_id in ($main_catgid)");
		$cnt=$query->num_rows();
		if($query->num_rows()>0)
		{
			foreach($query->result_array() as $res)  
		...
		$this->db->cache_on();
		//-------------------sujit end----------------------------//
		//$this->Product_descrp_model->catch_data_insert($main_catgid); 
		
	
	}
	//sujit end for category_catalog_cacheclear
	...
	function filter_product_list()
	{
		...
		//echo '<pre>';print_r($checked_product);
		$cnt_arr=count($checked_product);
		$fqq='';
		for($i=0;$i<$cnt_arr;$i++)
		{
			
			//echo $ordered_id[$i];exit;
			list($v1, $v2,$v3) = explode("|", $checked_product[$i], 3);
			//print_r($v1);exit;
			//echo $fqq.'start'.'<br>';
			//echo strpos($fqq,$v2).'<br>';
	...
	function product_detail()
	{
		...
		{		
				...
				if(@$this->session->userdata['session_data']['user_id']){
					...
					else{$this->load->view('single_product',$p);}
				}else{
	...
	function logintobuynowaddtocart_temp()
	{
		...
		$this->Product_descrp_model->insert_addtocart_temp($product_id,$sku_id);
		//$this->session->userdata['addtocart_sku'],$sku_id;
		//$this->session->set_userdata('logintobuysku_id',$sku_id);
		//redirect('Product_description/load_product_descrp/'.$product_id.'/'.$sku_id);
	...
	function seller_review_refresh_pg(){
		...
		$querycacheid=$this->db->query("SELECT lvl2 FROM cornjob_productsearch WHERE seller_id='$seller_id' AND prod_status = 'Active' AND status='Enabled' AND seller_status='Active' group by lvl2");
		$this->db->cache_on();
		$dskcatgid_idarr=$querycacheid->result_array();
		
		foreach ($dskcatgid_idarr as $sub) {
  		$tmpArr[] = implode(',', $sub);
		}
		$dskcatgid_id = implode(',', $tmpArr);
		...
		else{$this->load->view('seller_review_pg',$p);}
...
/*----------------------------------- 10-oct-2017 ----------------------------------- */	
	...
	function mtree2()
	{
		...
		//$p['id']=$id;
		//$Category_Lvl3_Id=$this->input->post('Category_Lvl3_Id');
		//$p['category_id']=$Category_Lvl3_Id;
...
/*--------------------------------------23oct17 price from-to end----------------------------------------------*/
	function suggestword()
	{
        ...
        $curl_strng=SOLR_BASE_URL.SOLR_CORE_NAME."/select?indent=on&q=".$search_title."&facet=true&facet.field=Category_Lvl3&facet.field=Category_Lvl2&facet.field=Category_Lvl1&facet.mincount=1&wt=json&rows=1&start=0";
			...
			{
				//echo $product_data['spellcheck']['collations'][1]['collationQuery'];exit;
				//echo $x='earphone';exit;
				...
				if($cnt>1)
				{
					...
					for($i=1;$i<$cnt;$i+=2){
						//echo $product_data['spellcheck']['collations'][$i]['collationQuery']['hits'];exit;
	...
	function arrck()
	{
	  ...
	  if($cnt_brand>0){
		 ...
		 for($i_arrbrand=0; $i_arrbrand<$cnt_brand; $i_arrbrand++ ) {
			...
			//echo '<pre>';print_r($textbox_val);
			//echo '<pre>';print_r($textbox_id);
			//echo '<pre>';print_r($afterckbox_val);
		...
		if (strpos($fqq, '&fq=') !== false) {
			...
			if(strpos($fqq, 'Brand:') !== false)
			{
			  ...
			  //echo $fqq;exit;
				...
				//$fqq=substr($fqq, strpos($fqq, "Brand:"));
				//echo $fqq;exit;
			 }
		//echo $fqq;exit;
