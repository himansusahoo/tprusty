	...
	$.fn.collapsibleCheckboxTree = function(options) {
		...
		this.each(function() {
			...
			if (defaults.includeButtons) {
				$(this).before('<button id="expand" class="btns" >Expand All</button><button id="collapse" class="btns" >Collapse All</button>');
			}
			...
			$("li:has(> ul:not(.hide)) > span", $(this)).addClass('expanded').html('<i class="icon-folder-open"></i>');
			$("li:has(> ul.hide) > span", $(this)).addClass('collapsed').html('<i class="icon-folder-close"></i>');
			...
			$("input[type='checkbox']", $(this)).click(function(ev){
				...
				if ($(this).is(":checked")) {
					...
					// Update the tree
					$("> span.collapsed", $(this).parent("li")).removeClass("collapsed").addClass("expanded").html('<i class="icon-folder-open"></i>');
				...
				// If unchecking...
					...
					if (defaults.uncheckChildren || (defaults.shiftClickEffectChildren && ev.shiftKey)) {
						...
						// Update the tree
						$("span.expanded", $(this).parent("li")).removeClass("expanded").addClass("collapsed").html('<i class="icon-folder-close"></i>');
					}
				}
			...
			$("li:has(> ul) span", $(this)).click(function(){
				...
				if ($(this).is(".collapsed")) {
					...
					$("> ul", $(this).parent("li")).removeClass('hide');
					// ... and update the html
					$(this).removeClass("collapsed").addClass("expanded").html('<i class="icon-folder-open"></i>');
				...
				// If was previously expanded...
					...
					$("> ul", $(this).parent("li")).addClass('hide');
					// and update the html
					$(this).removeClass("expanded").addClass("collapsed").html('<i class="icon-folder-close"></i>');
				}
			...
			$("#expand").click(function () {
				...
				// and update the html
				$("li:has(> ul) > span", $root).removeClass("collapsed").addClass("expanded").html('<i class="icon-folder-open"></i>');
				return false;
			});
			// Collapse all
			$("#collapse").click(function () {
				...
				// and update the html
				$("li:has(> ul) > span", $root).removeClass("expanded").addClass("collapsed").html('<i class="icon-folder-close"></i>');
				return false;
			});
			...
			$("#default").click(function () {
				...
				// and update the html
				$("li:has(> ul:not(.hide)) > span", $root).removeClass('collapsed').addClass('expanded').html('<i class="icon-folder-close"></i>');
				$("li:has(> ul.hide) > span", $root).removeClass('expanded').addClass('collapsed').html('<i class="icon-folder-close"></i>');
				return false;
			});
